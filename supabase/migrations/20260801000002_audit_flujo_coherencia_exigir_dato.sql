-- audit_flujo_coherencia v3 — el guardián reconoce cuando el motor ya exige el dato.
--
-- QUÉ CAMBIA Y POR QUÉ
-- La regla 6 (`default_cierra_el_negocio`) describe un riesgo condicional: *si falta el
-- dato*, el camino por defecto empuja el negocio a la etapa de cierre. Es lo que pasó con
-- los 17 casos que salieron de Entrega hacia Facturación.
--
-- Desde el objetivo O4 una etapa puede declarar `config_extra.exigir_dato_de_decision = true`
-- y entonces el motor NO evalúa la bifurcación sin el dato: frena el avance y dice qué campo
-- falta y dónde se responde. Con eso, la premisa del hallazgo ("si falta el dato") deja de
-- poder ocurrir por esa vía, y el default vuelve a ser lo que dice ser: una ruta deliberada,
-- la que toman los casos a los que la pregunta no les aplica (en SOENA, los que no llevan
-- devolución de IVA; su `label_default` ya lo declara).
--
-- Se deja el hallazgo vivo para toda etapa que NO lo declare, que es el estado por defecto de
-- todas las líneas de todos los workspaces. El flag no es una forma de silenciar al guardián:
-- es lo que enciende el freno en el motor. Silenciarlo sin encender el freno exigiría
-- escribir el flag y no desplegar el código — por eso migración y despliegue van pegados.
--
-- Reglas sin cambio: 1, 1b, 2, 3, 4, 5 y 7. En particular, una etapa que exige el dato
-- SIGUE siendo reportada si su campo decisorio no es obligatorio, no vive en un bloque que
-- retiene o usa un control binario: el freno del motor es la última barrera, no un permiso
-- para configurar mal lo de arriba.
...
comment on function audit_flujo_coherencia(uuid) is
  ...
  'exigir_dato_de_decision no reporta que su default cierre el negocio: el motor no evalúa '
  'la bifurcación sin el dato. '
