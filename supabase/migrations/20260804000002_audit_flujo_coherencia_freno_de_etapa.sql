-- audit_flujo_coherencia v4 — el guardián acepta el freno del motor también en la regla 3.
--
-- QUÉ CAMBIA Y POR QUÉ
-- La regla 3 (`decision_en_bloque_sin_gate`) exige que el campo que decide una ruta viva en
-- un bloque que retiene. La v3 ya reconoció `exigir_dato_de_decision` como freno válido, pero
-- SOLO en la regla 6. Resultado: una etapa con el flag encendido seguía siendo reportada por
-- la regla 3, y quitarle el gate al bloque —que es justo lo que el flag permite hacer— dejaba
-- un hallazgo ALTO permanente por una decisión deliberada.
--
-- Un control que grita por lo que el equipo decidió a propósito enseña a ignorarlo, y el
-- siguiente hallazgo real se pierde entre el ruido.
--
-- LO QUE HABILITA
-- Un bloque de SOLO LECTURA no debería ser gate: no se puede responder en pantalla, así que
-- cuando su dato no llega, retiene un caso sin ofrecer salida. Es lo que pasó en SOENA con el
-- bloque "Cita DIAN" de Entrega: 5 negocios frenados el 2026-08-04 con la respuesta ya
-- escrita en el propio bloque. Con el flag encendido el freno vive en el motor, que sí sabe
-- decir QUÉ falta y DÓNDE se responde, y sabe no exigírselo a quien la pregunta no le aplica.
--
-- POR QUÉ NO ES UN INTERRUPTOR PARA SILENCIAR
-- El flag no es una anotación cosmética: es lo que enciende el freno real en
-- `cambiarEtapaNegocioConGate`. Encenderlo sin desplegar el código exigiría escribir el flag
-- a mano en la etapa; por eso migración y despliegue van pegados. Y el freno del motor es
-- levantable por owner/admin con motivo en el log, igual que cualquier otro gate.
--
-- Reglas sin cambio: 1, 1b, 2, 4, 5, 6 y 7. En particular sigue reportándose el campo
-- decisorio que NO es obligatorio o que usa un control binario: el freno del motor es la
-- última barrera, no un permiso para configurar mal lo de arriba.
--
-- ⚠️ La exención se mide en la etapa que DECIDE, no en la que aloja el bloque. Es la
-- distinción que importa cuando el routing lee de otra etapa (`source_etapa_orden`): en SOENA
-- el campo `requiere_cita_dian` se responde en Documentación (6) y decide en Cobro (11), y el
-- freno que lo protege es el de Cobro.
...
as $$
  ...
  exigible as (
    select
      ...
      -- ¿La etapa que decide frena por su cuenta mientras el dato falte?
      coalesce((ed.config_extra->>'exigir_dato_de_decision')::boolean, false) as motor_exige_dato
    from dueno d
    join etapas_negocio ed on ed.linea_id = p_linea_id and ed.orden = d.etapa_decide
  ...
  --
  --    Salvo que la etapa que decide declare `exigir_dato_de_decision`: entonces el freno
  --    no depende del bloque, lo aplica el motor, que además nombra el campo que falta y
  --    dónde se responde, y no se lo exige a quien la pregunta no le aplica. Es lo que
  --    permite que un bloque de SOLO LECTURA deje de ser gate sin abrir un hueco: un gate
  --    que nadie puede responder en pantalla retiene sin ofrecer salida.
  ...
  --
  --    Salvo que la etapa declare `exigir_dato_de_decision`: entonces el motor frena el
  --    avance mientras el dato falte, así que por esta vía el default ya no puede tomarse
  --    con un vacío. Lo toman solo los casos a los que la pregunta no les aplica, que es
  --    para lo que existe. Sin ese flag —el estado de todas las líneas por defecto— el
  --    hallazgo sigue vivo.
  ...
  where e.linea_id = p_linea_id and e.is_active
    ...
    and (e.config_extra->>'exigir_dato_de_decision')::boolean is distinct from true
...
comment on function audit_flujo_coherencia(uuid) is
  ...
  'esas tres exigencias se le piden a la pregunta de la que depende, no a él. Una etapa con '
  'exigir_dato_de_decision no reporta que su default cierre el negocio ni que su bloque no '
  'sea gate: el motor no evalúa la bifurcación sin el dato, y ese freno nombra el campo que '
  'falta. Sigue reportándose el campo no obligatorio o con control binario. '
