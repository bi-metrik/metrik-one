-- La fecha de la cita DIAN se lee en la zona de Colombia, no en UTC.
--
-- El bloque `fecha_cita_dian` guarda una fecha sin hora. La RPC la convertia con
-- `::timestamptz`, que usa la zona de la sesion; en esta base es UTC. Verificado:
-- `'2026-08-13'::timestamptz` = 13-ago 00:00 UTC = **12-ago 7:00 p.m. en Bogota**.
-- El plazo real quedaba cinco horas mas estricto que la medianoche que se quiso
-- escribir, y en contra del operativo siempre.
--
-- Medido en SOENA (agosto 2026) antes de aplicarlo: ningun caso cae en esa franja,
-- asi que **no cambia ningun veredicto del mes en curso**. Es deuda que muerde
-- cualquier mes siguiente, y de la misma familia del gotcha ya documentado con
-- `todayBogotaISO()`: Vercel y Postgres corren en UTC, Colombia es UTC-5.
--
-- ⚠️ Lo que este cambio NO resuelve, y es decision de Deisy: la cita no trae HORA,
-- asi que "36h antes de la cita" sigue significando "36h antes de la medianoche".
-- Quien envia la manana misma de la cita cuenta como tarde (V0130 a las 09:26 y
-- V0215 a las 09:15, agosto). Si la hora importa, hay que capturarla en el bloque,
-- y eso es trabajo de flujo, no de tablero.
...
-- ============================================================
-- ROLLBACK (comentado):
--   Re-aplicar la definicion de 20260818000001_bono_correcciones_sin_evidencia.sql
-- ============================================================
