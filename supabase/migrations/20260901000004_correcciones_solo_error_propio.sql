-- El indicador de correcciones DIAN castiga solo el error PROPIO, igual que calidad.
--
-- Decision de Mauricio, 2026-08-31. Hasta hoy el numerador contaba TODA devolucion
-- de la DIAN atribuida a una persona, sin mirar `causa`. Calidad, en cambio, filtra
-- `causa = 'error_propio'` desde que nacio el modulo. Eran dos varas distintas para
-- la misma clase de hecho, y la mas dura era la que nadie habia acordado.
--
-- Medido en produccion (SOENA, agosto 2026). Las 6 devoluciones del mes:
--   V0206, V0244, V0213, V0403  criterio_tercero, sin atribuir
--   V0234                       criterio_tercero, Jhon Fredy
--   V0017                       error_propio,     Maria Camila
--
-- O sea que la unica devolucion que le pesaba a Jhon Fredy es la que el propio
-- registro describe como ajena: "se le agendo cita pero la DIAN le dijo que no
-- tenia cita". Le costaba los 20 puntos completos del indicador.
--
-- Efecto medido, antes y despues, con el resto de la configuracion igual
-- (piso 0,90, envio suspendido):
--   Jhon Fredy    7/8 = 87,5% -> 0 pts   pasa a   8/8 = 100% -> 20 pts   $240.000 -> $360.000
--   Maria Camila  7/8 = 87,5% -> 0 pts   sigue en 7/8 = 87,5% -> 0 pts   $350.769 sin cambio
-- Maria Camila no se mueve porque su devolucion (V0017, mandar la orden de compra
-- en vez de la factura) si esta marcada como error propio.
--
-- Que NO cambia, y es deliberado: la COBERTURA. `cobertura_correcciones` sigue
-- contando devoluciones de cualquier causa, porque es exactamente la forma de
-- calidad: su cobertura pregunta "se uso el mecanismo este mes", no "alguien
-- fallo". Si la cobertura exigiera un error propio, un mes limpio de verdad
-- quedaria sin medir y no pagaria nunca, que es justo la trampa que el guard de
-- cobertura existe para evitar.
--
-- Se agrega ademas lo que el indicador DESCARTA: `terceros` por persona y
-- `devoluciones_error_propio` del mes. Sin ese dato, un mes con devoluciones a la
-- vista y el indicador en 100% solo se puede leer como "no hubo devoluciones", que
-- es falso. La pantalla tiene que poder decir cuantas hubo y por que no cuentan.
--
