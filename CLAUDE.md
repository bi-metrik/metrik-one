...
- **Pendiente, fuera del alcance del frente:** el schema `cliente_reposteria` tiene 5 tablas **sin RLS** con grants a `anon` (solo `productos`, 14 filas, tiene datos). Se le quitaron los privilegios sobrantes, pero **no se le activo RLS**: no consta quien lo consume y activarlo a ciegas puede tumbar ese piloto. Decision de Mauricio.
