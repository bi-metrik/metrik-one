...
- **La marca dice el motivo REAL de cada borrador** (2026-09-23): «pantallazos por actualizar», «margen bajo el mínimo», «IVA sin calcular», «IVA incluido sin plantilla»; dos juntos se leen enteros, tres o más como el principal «y N más». Hasta ese día los dos del IVA salían diciendo «margen». **La lista, las etiquetas y el aviso salen de UN módulo** (`src/lib/cotizaciones/motivos-borrador.ts`, puro, lo importan también los avisos del editor): no decide CUÁNDO es borrador, solo nombra las condiciones ya decididas. `ponerMarcaDeBorrador` exige la lista: el motivo por defecto era justo el defecto.
...
- **La confirmación vieja se queda (rubros intactos) pero `confirmacionDesactualizada` la marca**: no se reimprime el reparto por pasajero (editor y PDF) y no se reconfirma hasta pegar la captura nueva. Aviso a nivel de cotización (`captura-desactualizada.ts`).
- **⚠️ Con una captura o confirmación desactualizada la cotización NO sale de borrador** (Mauricio, 2026-09-22): lo niega el SERVIDOR (`captura-desactualizada-datos.ts`) en los dos «Enviar», en «Aprobar» desde borrador y en `updateCotizacion({ estado })`, antes del piso de margen. El botón del editor se apaga con el MISMO texto (`motivoParaNoEnviar`). **El PDF se descarga como BORRADOR** (#834 siguiente): marca de agua «pantallazos por actualizar · no enviar» en cada página (`ponerMarcaDeBorrador(pdf, motivos)`, sobre los bytes: no toca la plantilla), sin guardar ni registrar la salida, por los dos motores. **No frena:** aprobar una ya `enviada`, ni lo que no tiene `tarifa_pax` (ni lee el viaje). Si no puede leer líneas o pasajeros, frena (y el PDF sale borrador).
...
- **⚠️ El metadata por inquilino NO va en el layout raiz, va en `src/app/(marketing)/login/page.tsx`.** Meterle `headers()` al layout raiz volveria dinamico el render de toda la app. El `/login` del subdominio ya resolvia el branding sin sesion (slug -> workspace -> `name` + `logo_url`) y es exactamente lo que ve el rastreador, porque todo subdominio redirige ahi.
- **⚠️ `openGraph.url` NO se declara.** El mismo layout sirve todos los subdominios del comodin `*.metrikone.co` y una url fija mentiria sobre el enlace que se comparte.
- **⚠️ Satori (el motor de `ImageResponse` de `next/og`) NO implementa `object-fit`, `max-width`, ancho por contenido ni unidades `ch`.** En la primera prueba la placa se fue al `max-width` y el logo se quedo en su tamano intrinseco. Toda esa geometria se calcula a mano en `src/lib/og/medidas-tarjeta.ts` y a la imagen se le pasan alto y ancho explicitos. El `15ch` del artboard se convirtio midiendo el avance del glifo "0" en la instancia real: 491,4 px.
- **⚠️ Satori tampoco lee woff2, que es el formato de las fuentes autoalojadas del repo.** No se metio tipografia nueva: se parte de los mismos binarios y se recorta con `pyftsubset` (fontTools, ya disponible en la torre, con `brotli`) a los caracteres que la tarjeta imprime. **13.028 bytes** las tres, embebidas en base64 en `fuentes-tarjeta.ts` para no depender de que el empaquetado arrastre archivos. **Newsreader tiene eje `opsz` con default 18**, y Chrome le aplica `opsz=56` a 56px: hay que instanciar en `wght=400, opsz=56` o salen otras letras.
- **Los logos de clientes son JPEG progresivos** en varios casos. `resvg` si los digiere, verificado antes de construir nada; por eso `medirImagen` acepta todos los SOF y no solo `0xC0`.
- **⚠️ El middleware corta `/api/og/` antes de tocar Auth.** No es optimizacion: una respuesta con `Set-Cookie` no la cachea el CDN. Y la imagen se sirve **del dominio base**, no del subdominio, para que el CDN guarde una copia por inquilino y no una por enlace. Vercel consume y elimina `s-maxage` y `stale-while-revalidate` de la respuesta visible al cliente y deja solo `max-age=3600`: la prueba de que los honro es el `x-vercel-cache: HIT` con `age` creciendo.
- **La tarjeta no dice nada del negocio del cliente, a proposito.** Un workspace sin `logo_url` (2 de 17) o un slug que no resuelve caen en la generica; `/api/og/<slug desconocido>` responde 302 a la generica.
- **El lockup MeTRIK one va en contornos** (`src/lib/og/lockup-one.ts`), no tipografiado: asi la tilde de la "é" no depende de que cargue una fuente. La "é" es **minuscula** siempre.

### El recaudo del honorario pasa a ser condicion de entrada a la cola, y el retenido se ve con su razon - PRs #578 `30eba08`, #581 `bfc2036` (2026-09-09)

- **⚠️⚠️ Un caso con el honorario incompleto ya NO entra a la cola de facturacion.** Antes salia listado con el rotulo gris "Falta: recaudo del honorario" junto a las demas faltas, y ese rotulo tapaba **tres decisiones distintas** (anticipo del 50%, recaudo cero, descuadre de pocos pesos) bajo la misma frase. La regla vive en `src/lib/facturacion/caso-listo.ts`, el criterio unico que ya comparten servidor y pantalla para que el contador de la bandeja y la lista **no se puedan desincronizar**.
- **Tres estados, con una banda de materialidad:** `banda(honorario) = max(TOLERANCIA_SALDO_COP, round(0.01 * honorario))`. Debajo del piso -> `cubierto` (boton directo). Entre piso y banda -> `descuadre_menor`: se lista **marcado**, exige justificacion escrita y queda en `activity_log`. Encima de la banda -> `retenido`.
- **⚠️ `TOLERANCIA_SALDO_COP` no se toca.** Es un solo piso de materialidad para todo el sistema y su dueno es el CFO; la banda se construye **encima**, no lo reemplaza. Cambiarlo sigue siendo una decision de una sola persona en un solo archivo (`src/lib/negocios/tolerancia-saldo.ts`).
- **⚠️ La factura se emite por el honorario completo, nunca por lo recaudado.** El descuadre se marca, no descuenta.
- **⚠️ Una banda que parte un grupo esta mal calibrada.** Medida contra produccion antes de fijarla: el retenido mas cercano debe 11.8% y el unico caso dentro de la banda debe 0.47%. Ningun grupo queda cortado por el borde.
- **⚠️⚠️ Sacar algo del panel le quita EN SILENCIO todas sus acciones, no solo la que estorbaba.** El #578 quito de la vista a los retenidos y con eso les quito la **adopcion** (registrar una factura que ya existe en Siigo), que nada tenia que ver con el recaudo. El #581 lo corrige: seccion propia colapsable con su conteo, cada tarjeta con "Retenido: falta recaudar $425.000 de $850.000 (50%)" **en franja propia y no en chip pequeno**, sin boton de facturar, con adopcion, y encontrable por codigo. `totales.listos` y `totales.incompletos` **excluyen** retenidos, para que la pestana no se re-infle.
- **⚠️ Una prueba pura no fija que el JSX obedezca la decision.** Solo la prueba de render (`renderToStaticMarkup`) mata la mutacion "el boton de facturar ignora el helper"; las pruebas del helper siguen verdes con el boton mal pintado. Precedente: `recaudo-cambiado-banner.test.ts`.
- **⚠️ Las pruebas de render se quedan en `.ts`.** El `include` de `vitest.config.ts` es `src/**/*.test.ts`: renombrar a `.tsx` para usar JSX saca el archivo de la suite **en silencio**.
- **⚠️ Un PR en estado `CONFLICTING` no encola los checks obligatorios** y se ve como "sin checks", no como conflicto. Paso en #578 por la memoria de agente. Desde entonces la memoria va en **commit aparte** del codigo.
- Sin migraciones y sin escrituras a produccion en ninguno de los dos PR.

### Fuera el badge del sidebar de Tesoreria: los contadores viven en las pestanas - PR #585 `70263f1` (2026-09-09)

- **⚠️ Un badge de nav tiene que poder reconciliarse con algo al entrar.** El de Tesoreria contaba `sobrepagos sin conciliar + referencias duplicadas + negocios etiquetados a mano` (RPC `count_negocios_por_conciliar`), que no coincide con ninguna de las cuatro pestanas de la pantalla (Por confirmar, Saldos, Por facturar, Recibos de caja). El usuario entraba con un numero en la cabeza y no encontraba donde vivia.
- **⚠️⚠️ Una de sus tres patas estaba muerta.** Nada en el codigo escribe ya `solicitud_conciliacion` ni `conciliacion_atendida` (lo documenta la migracion `20260901000010`): los negocios etiquetados a mano en julio de 2026 sumaban **para siempre**, sin ninguna via desde la app para apagarlos. **Un contador cuya unica via de bajar es un evento que nadie emite no se puede resolver, solo ignorar.**
- **Se fue tambien la consulta.** `layout.tsx` llamaba la RPC en cada render del layout, o sea en **cada pantalla del producto**, no solo en Tesoreria.
- **Sin migracion:** `count_negocios_por_conciliar` y `count_negocios_por_conciliar_sin_guarda` siguen en la base. Las filas historicas de `activity_log` tambien.
- **⚠️ Un check verde certifica el arbol que existia cuando corrio.** Aqui: tipos en verde, luego un arreglo de lint (sacar `children` a tercer argumento de `createElement`) que rompio los tipos, y el rojo aparecio en CI porque no se repitio la corrida.
- Prueba de render nueva (`app-shell-tesoreria-render.test.ts`, 3 casos) que se vio caer reintroduciendo el badge a mano. **El shell se deja renderizar en aislamiento**: solo pide `usePathname` y `useRouter`; `createClient()` corre unicamente dentro de `handleSignOut`.

### El routing decidia con la respuesta de un bloque que al caso no le aplicaba - PR #586 `50a6b4f` (2026-09-08)

- **⚠️⚠️ El motor de rutas armaba su juego de valores con `Object.assign` sobre TODOS los bloques `datos` de la etapa fuente, sin mirar el `condition` ni el `desactivado` de ninguno.** Un bloque condicional que no le aplica al caso es **invisible en pantalla y decisivo en el motor**: nadie puede verlo ni corregirlo por la via normal. Estaba **duplicado en dos sitios** de `negocio-v2-actions.ts`, el avance de etapa y el salto encadenado por saldo.
- **El arreglo.** `src/lib/negocios/campos-de-routing.ts` (nuevo, puro) descarta los bloques `desactivado: true` y aquellos cuyo `condition` no se cumple, con cache por condicion. `camposDeRoutingDelNegocio` es el **unico** helper y lo usan los dos sitios. `dato-de-decision.ts` gana `destinoDeRouting`: el bucle que elige destino estaba escrito **tres** veces.
- **⚠️ La condicion NO se reevalua en TypeScript.** El evaluador entra por parametro y lo resuelve la RPC `condicion_cumplida`, la misma que usan los gates y que replica el render. Reimplementarla es la lista paralela que ya costo caro con las seccionales.
- **⚠️ El filtro va ANTES del gate de dato de decision**, no despues. Si no, el gate y la evaluacion de `routing.conditional` ven mapas distintos y un caso puede pasar el gate con un valor que el routing luego ignora.
- **⚠️ Una limpieza que solo corre en una etapa no es una defensa.** El guard `solo_si` del auto-init ya retiraba el campo huerfano y dejaba `_campo_retirado`, pero **nunca corrio**: el auto-init solo carga los `bloque_configs` de `etapa_actual_id`, asi que solo alcanza a los casos parados justo en esa etapa. Los que llegan de un cargue ya pasados de ahi quedan fuera de alcance para siempre.
- **Lo que costo (SOENA, V0431).** `cita_dian_requerida` (condition `servicio = solo_iva`) tenia `false` en un caso `completo`: el routing de Cartera lo leyo y el negocio salto de Certificacion (9) a Anexos (18), saltandose Segundo cobro, Cartera, **Entrega** y **Cita**. **177 negocios abiertos** con el mismo valor huerfano. Radio del arreglo, medido en los 16 workspaces: **un solo routing cambia**, y los 19 casos a los que el bloque SI aplica mantienen destino exacto.
- **Error encadenado, en el cargue.** `scripts/cargue-historico-iva.ts:380` sembraba el bloque en todos los casos, con lista de seccionales hardcodeada y sin el respaldo al RUT que si tiene la linea 324: **una seccional ausente se grababa como un `false` afirmativo**. Ahora solo siembra si `servicio === 'solo_iva'`, resuelve con `canonizarSeccional` + RUT, y **si no resuelve deja el campo vacio**.
- 13 pruebas en `campos-de-routing.test.ts` con fixtures de produccion; **5 de 13 vistas caer** por mutacion al quitar los dos filtros. Sin migracion y sin escrituras a produccion.

### Un contador de "emitibles" que sumaba avisos como si fueran bloqueos, y una tarjeta que nombraba consecuencias en vez de la causa - PRs #572 `aa80ed1`, #573 `963f3d9` (2026-09-08)

- **⚠️⚠️ El control de recibos decia 19 emitibles de 47 y el real es 42.** Escondia mas de la mitad del trabajo disponible sin fallar en nada. Contaba como bloqueantes dos cosas que no frenan la emision: el **tercero en Siigo** (lo crea `asegurarClienteSiigo` desde el RUT **dentro de la misma llamada** que emite) y el **correo del cliente** (solo decide si al cliente se le avisa, la emision sale igual). **Un faltante que no impide la accion no va en la lista de faltantes.**
- **El arreglo (#573, `src/lib/actions/recibos-control-actions.ts`).** `faltantes` conserva un solo elemento, `RUT del cliente`. Lo demas pasa a un campo nuevo, `avisos`, que `conciliacion/tab-recibos.tsx` pinta **al lado** del boton "Emitir recibo" en vez de reemplazarlo: la pantalla informa sin quitar la accion.
- **⚠️ La identidad se resuelve en dos pasos, no en uno.** Primero la marca `negocios.metadata.siigo_cliente.identificacion`; solo para los pendientes **sin** marca se lee el bloque RUT. En SOENA eso baja la lectura de ~300 negocios a ~17. Sin ese filtro, la pestana leeria la linea entera en cada carga.
- **⚠️ Los campos extraidos de un documento viven en `data->'campos'->'<slug>'->>'value'`**, no colgando de la raiz del bloque. Consultar `rut->'numero_identificacion'->>'value'` devuelve `null` en **todas** las filas y parece que el dato no existe. Se ve con `jsonb_pretty` antes de escribir la consulta.
- **#572 - la cola de facturacion nombra la causa.** Mostraba cuatro faltantes de cliente (identificacion, nombre, direccion, ciudad DANE) que se leen como cuatro campos por teclear. Los cuatro salen del **mismo** documento. Campo `sin_rut` en `CasoPorFacturar` y una sola etiqueta, **"RUT sin cargar"**, en rojo con tooltip. Los faltantes de la **factura** se conservan aparte: esos no salen del RUT. **`casoListoParaFacturar` no cambio**: el gate es el mismo, cambio como se explica.
- **⚠️ `sin_rut` cubre dos casos distintos a proposito**: sin documento cargado, y con documento cargado del que no se pudo extraer la cedula o el NIT. El mapa solo guarda RUT con identificacion utilizable, asi que su ausencia significa las dos cosas.
- Las 9 pruebas nuevas (4 en `caso-listo.test.ts`, 5 en `recibos-control.test.ts`) se vieron **caer** contra la implementacion anterior por mutacion aplicada y revertida. Sin migraciones y sin escrituras a produccion en ninguno de los dos PR.

### El aviso de recaudo cambiado congelaba el negocio para siempre: nadie podia resolverlo — PR #569 `033ac76` (2026-09-08)

- **El defecto.** `redistribuirReferencia` (`src/lib/actions/conciliacion-actions.ts`) pega el aviso `negocios.metadata.recaudo_cambiado_pendiente` a cada negocio afectado. Es un gate duro en `cambiarEtapaNegocioConGate` que **no cede al override de owner** (correcto, decision del 2026-08-11). Lo roto: se ponia **incondicionalmente**, y **ninguna pantalla podia quitarlo**. `resolverAviso` solo colgaba de `aplicarRetrocesoFinanciero`, que no la llamaba ningun `.tsx`: codigo muerto. Resultado, cualquier negocio con reparto corregido quedaba congelado de forma permanente, en **todo** workspace con `modules.conciliacion`.
- **⚠️ Un gate sin salida en la interfaz no es un control, es una trampa.** El caso real (SOENA V0442/V0443) tenia la plata perfectamente repartida y $17.188 a favor en cada uno. El aviso traia `gatesReabiertos: 0` y `destinoSugerido: null`: el propio motor sabia que no habia nada que reabrir ni a donde devolver, y lo ponia igual. **Al agregar un gate, escribir su salida en la misma sesion.**
- **⚠️ Un boton de override que no aplica es peor que no tenerlo.** El modal ofrecia "Omitir gate (owner)" sobre un gate que corre fuera del bloque `if (!motivoOverride)`. Ahora el bloqueo viaja con `omitible: false` y el modal lo esconde.
- **El arreglo.** `avisoRecaudoNecesario` (puro, en `src/lib/negocios/retroceso-financiero.ts`) decide si el aviso hace falta: solo si el negocio quedo **corto**. Banner `recaudo-cambiado-banner.tsx` en la ficha con boton de resolver bajo `ctxFinanciero()`, que ademas conecta `proponerRetroceso` y `aplicarRetrocesoFinanciero`. Sin migracion: el aviso ya vivia en `metadata`.
- **⚠️ El liston de "cuadrado" es `valorARecaudar` (honorario + tarifa pasante), no lo exigible en la etapa.** Deliberado: quien cubre la cuenta entera no puede estar corto en ninguna etapa, asi que la respuesta no depende del routing. La alternativa (`escalonesDelNegocio`) abre un punto ciego, un negocio muy avanzado que pierde plata hasta quedar solo con el anticipo pasaria sin aviso, y `gates_reabiertos` **no lo delata** porque `recalcularNegocioPorCambioDeRecaudo` corta antes si `anticipoCubiertoPorSaldo`.
- **⚠️ Para probar donde QUEDA un aviso, el doble tiene que escribir.** Con un doble de solo lectura, "no se puso" y "se puso y no se ve" son indistinguibles. `test/redistribucion-doble.ts` corre `redistribuirReferencia` completa contra un doble que persiste; los 7 casos cayeron en las dos direcciones (con el aviso incondicional y con el aviso apagado).
- Primer test de render del repo (`renderToStaticMarkup`, sin DOM): verifica que el banner se pinta, que texto saca y que el comercial lo ve **sin** botones.

### Cotizacion aceptada corregible, y el motor de presupuesto vs ejecutado que comparaba contra el numero equivocado — PRs #526 `196e09e`, #529 `866f951`, #532 `ecfc669` (2026-09-03 a 09-04)

- **#526 — una cotizacion `aceptada` deja de ser terminal.** Boton "Corregir" que la devuelve a borrador, pone `precio_aprobado` en NULL y reabre el bloque. Doble candado: rol gerencial (`puedeCorregirDocumentos` de `src/lib/roles.ts`, no la lista local de `reproceso-control.tsx`) **y** que el negocio no haya avanzado de etapa **ni** tenga cobros con `fecha`. Si hay plata recibida, soltar el precio deja el saldo apuntando a un precio que ya no existe: ahi toca duplicar.
- **⚠️ El modo de un bloque no lo decide `isEditable()` del documento, sino `getBloqueMode()`.** Un bloque `completo` llega siempre como `'visible'`, aun en la etapa donde seria editable. Condicionar un boton a `modo === 'editable'` es garantizar que nunca aparezca. La condicion correcta se arma con la etapa (`etapa-editable.ts`), no con el modo.
- **⚠️⚠️ #529 — el presupuesto ignoraba `items.cantidad`.** Sumaba `rubros.valor_total` sin multiplicar y sin siquiera leer la columna. Caso real: 794 unidades mostraban **$71.132 en vez de $56.479.070**, o sea que cualquier gasto pintaba rojo. El numero bueno ya existia en `cotizaciones.costo_total`; la formula del editor (`actualizarTotalesCotizacion`) era la referencia. Los items `es_ajuste` quedan fuera: son cuadre de precio (AIU o descuento), no costo.
- **⚠️⚠️ La barra "Total" dividia entre el PRECIO de venta, no entre el presupuesto de costo.** Con eso un caso podia gastarse el doble de su costo y seguir en verde, porque los umbrales median margen consumido y no sobrecosto. **Comparar costo contra costo; el margen es otra pregunta y va con nombre propio.**
- **⚠️⚠️ El mapa `TIPO_A_CATEGORIA` estaba casi todo roto.** Los tipos vivos en `rubros` son `materiales`, `mo_propia`, `mo_terceros`, `servicios_prof` y `viaticos`; el codigo declaraba `mano_obra`, `servicio`, `transporte` y `otro`. **De cinco tipos reales solo coincidia uno**, asi que casi toda barra leia ejecutado 0 y nadie lo noto: un mapeo equivocado no falla, miente en silencio.
- **⚠️ `.find(c => c.estado === 'aceptada')` no tomaba una al azar: tomaba la mas reciente**, por un `.order('created_at', desc)` puesto mil lineas antes. Implicito es peor que aleatorio. Ya hay un negocio con dos aceptadas a la vez (17 minutos aparte); el #532 lo advierte en pantalla.
- **#532 — lo que no se puede medir se declara, no se rellena.** Gasto sin rubro presupuestado: fila propia sin barra, para que la suma reconcilie con el KPI. Horas que entran valiendo $0 (`salary/160` con default 0, `staff_id` nulo): aviso con el conteo. Negocio sin cotizacion aceptada: se explica el vacio y se muestra el ejecutado. **Se descarto usar un borrador como linea base y hardcodear una tarifa por hora**, que era exactamente la mentira que se estaba quitando.
- **⚠️ Contar la poblacion de la PANTALLA, no la de la tabla.** El diagnostico de entrada citaba 159 gastos y 39 horas de toda la base; el bloque de Ejecucion existe en **7 negocios de 4 workspaces** y SOENA no tiene ni una instancia. La mitad de las cifras no aplicaba, y una falla que parecia urgente resulto latente.
- Logica en el helper puro `src/lib/negocios/presupuesto-ejecucion.ts` con sus pruebas; `getNegocioDetalle` y `BloqueEjecucion.tsx` solo consumen. Sin migracion y sin una escritura a produccion en los tres PR.
- **Suelto:** `cotizaciones.costo_total` puede ser 0 con items presentes (cotizar sin desglosar rubros). Aprobar una asi deja la seccion sin presupuesto; el #532 lo explica, pero el problema es de captura, no de pantalla.

### Descarga a Excel de la tabla de negocios filtrada (genérico, gate por rol) — PR #525 `59eeed6` (2026-09-03)
- Botón "Descargar Excel" en `/negocios` (owner/admin/supervisor, `puedeDescargarNegocios` en `src/lib/roles.ts`). El cliente manda los ids visibles tras `aplicarFiltros` a `POST /api/negocios/export`; el server rearma la fila: lo que baja es lo que se ve.
- Fila ancha (49 columnas): `NegocioResumen` + `v_negocio_valor` + `v_negocio_bonificable` + responsables por rol + cobros ordenados (primer/segundo/otros, anulados fuera). Módulo puro `src/lib/negocios/export-excel.ts` con tests.
- Gotchas: las vistas de dinero son server-only → `createServiceClient()` + `.eq('workspace_id')`; el recaudado se lee bruto de `v_cobro_valor.a_tramo1 + a_tramo2` (la columna `honorario_recaudado` de `v_venta_mes_comercial` bajó a base el 2026-09-02); SheetJS escribe fechas con getters locales → `Date` armado con hora de pared de Bogotá y `cellDates: true`.
- Workspace-agnóstico: las columnas de caso salen de `config_extra.negocio_card`, como la tarjeta.


**Sesion:** 2026-09-03 (`soena/ve` → producto: **el tablero de marketing, y un sync que quedo desplegado sin poder correr**). PRs **[#509](https://github.com/bi-metrik/metrik-one/pull/509)**, **[#510](https://github.com/bi-metrik/metrik-one/pull/510)**, **[#512](https://github.com/bi-metrik/metrik-one/pull/512)** y **[#516](https://github.com/bi-metrik/metrik-one/pull/516)** mergeados con checks verdes. Migracion `20260903000001_marketing_campana.sql` aplicada: **DDL puro** (tabla `campana_insights`, vistas `v_marketing_negocio` y `v_marketing_campana`), sin una sola escritura de datos.

- **Pestana Marketing nueva en `/tableros`** (opt-in `modules.marketing_campanas`), con dos lentes: **mes de venta** (cuadra contra caja) y **cohorte de campana** (la unica con la que "que tan rentable es la campana" tiene respuesta, porque un lead de julio puede cerrar en septiembre). Reglas puras y probadas en `src/lib/tableros/marketing.ts`; edge function `meta-insights-sync` para el gasto.
- **⚠️⚠️ El `campaign_name` que guarda un lead es una FOTO del momento, y Meta renombra las campanas despues.** Medido: lo que ONE guarda como `CLIENTES POTENCIALES AGO 2026 PLUS` (304 leads, la mas grande de SOENA) hoy se llama en Meta `CLIENTES POTENCIALES AGO ($100)`. **Agrupar por nombre parte la campana en dos filas** el dia que alguien la renombre, cada una con la mitad del gasto, y las dos se leen como campanas distintas. **Unir siempre por `campaign_id`; el nombre es etiqueta, y la que se pinta es la vigente en Meta.**
- **⚠️ Una metrica que puede calcularse sobre cero evidencias necesita decidir a proposito que muestra.** `gastoConocido` se apoya en `sincronizado_at`, no en `gasto > 0`: CPL, CAC y ROAS devuelven `null` y la pantalla pinta **raya con un aviso**. Un `$0` de inversion se lee como "no gastamos", que es una afirmacion; lo unico cierto seria "no lo sabemos". Misma familia que el indicador sin datos del bono.
- **⚠️ El system user de Meta NO puede enumerar cuentas publicitarias, pero SI leer una campana por su id.** `assigned_ad_accounts` devuelve vacio y los endpoints del business piden `business_management`. Leer por id alcanza y ademas cubre las **dos** cuentas de SOENA sin tener que saber cuales son, porque los ids salen de los leads que ya estan en la base. Consecuencia asumida: una campana que nunca trajo un lead a ONE es invisible para el sync.
- **⚠️⚠️ Un secreto que vive en los secretos del PROYECTO deja una automatizacion inarrancable.** `meta-insights-sync` se autenticaba solo con un secreto compartido, y esos solo se escriben desde el panel o con el CLI autenticado: quedo desplegado y sin poder correr, porque encender el cron dependia de que una persona entrara a una pantalla. El **#516** le agrega una segunda credencial: la **service role key** en `Authorization: Bearer`, que Supabase inyecta sola en toda edge function (`getServiceClient()` ya depende de eso) y el cron lee del vault. **NO afloja la puerta** —quien tenga esa key ya puede escribir la tabla directo contra la base— y **la alternativa que se descarto SI la habria aflojado**: `verify_jwt: true` sin secreto deja entrar a cualquiera con la anon key, que viaja en el bundle del navegador. **Regla general: al elegir donde vive el secreto de un proceso automatico, preguntar quien va a poder escribirlo despues.**
- **⚠️ La guarda que mas importa en una comprobacion de credenciales es que un valor AUSENTE nunca autorice.** Sin `Boolean(esperado) &&`, un cuerpo sin secreto contra un entorno sin secreto configurado da `undefined === undefined` y entra. `credencialValida` vive en el modulo puro con 7 pruebas, y esa guarda se verifico **mutando la linea y viendo caer el test** — el resto de las pruebas seguian en verde.
- **⚠️ `main` y produccion quedaron DESALINEADOS y hay que cerrarlo:** el #516 esta mergeado pero la edge function desplegada sigue en **version 1**, asi que el cron todavia no puede autenticarse. La pestana funciona igual, con la inversion en raya. Pasos en `proyectos/soena/ve/2026-09-03_activar-sync-marketing.md`.
- **⚠️ HALLAZGO ABIERTO — `v_negocio_atribucion` es LAST-touch, no first-touch.** Resuelve con `order by ci2.ocurrida_at DESC limit 1` y **nunca mira `custom_data.origen`**, pese a que la documentacion afirmaba lo contrario. Medido: de 79 negocios atribuidos, **6 cambiarian de campana**, y **V0024 esta atribuido a una campana cuya unica interaccion es un mes POSTERIOR a su venta**. El sesgo crece y favorece siempre a la campana que esta corriendo, que es justo la que se esta juzgando. **No se corrige desde el tablero:** esa vista tambien alimenta el panel de origen del lead y `comision_retenida`, que mueve plata hacia promotores.

**Sesion:** 2026-09-02 a 03 (`soena/ve` → producto: **el telefono solo admite numeros, y el formulario de Meta se entiende solo**). PRs **[#507](https://github.com/bi-metrik/metrik-one/pull/507)** (`adbcae2`) y **[#511](https://github.com/bi-metrik/metrik-one/pull/511)** (`bf991fc`) mergeados con checks verdes. Migraciones `20260902230000_telefono_solo_numeros_y_usuario_whatsapp.sql` y `20260903000001_field_map_aprendido_por_formulario.sql` aplicadas y registradas en el ledger. Columna nueva `contactos.usuario_whatsapp`.

- **⚠️⚠️ Un CHECK constraint que llama a una funcion rompe el restore de `pg_dump`.** pg_dump escribe las constraints **dentro del `CREATE TABLE`**, o sea antes de que existan las funciones, y el restore falla. Un trigger se crea **despues** de los datos, asi que si puede llamar funciones. Ademas un trigger puede **limpiar antes de juzgar**, que es lo que un CHECK nunca hace. Regla: **toda regla de dominio que necesite una funcion va en trigger, no en CHECK.**
- **⚠️⚠️ Validar sin limpiar primero borra datos buenos.** De los 11 telefonos que no pasaban la regla, **uno era un numero perfectamente valido con un U+202C pegado** de copiar y pegar desde WhatsApp. `limpiar_telefono` **nunca rechaza**: solo arregla accidentes probados (U+00A0, U+200B..U+200F, U+202A..U+202E, U+2066..U+2069, U+FEFF, espacios repetidos, cola decimal de Excel). El juicio lo hace `telefono_valido`, que **nunca modifica**. Las dos funciones separadas a proposito.
- **Los invisibles Unicode se escriben `chr(160)` / `chr(8203)`, nunca literales.** Un caracter invisible dentro del archivo de migracion no se puede revisar en el PR ni verificar en transito: los escapes `​` se normalizaron a caracteres literales al pasar por el MCP. `chr(n)` es ASCII inequivoco en la fuente. `translate(str, from, to)` **borra** los caracteres de `from` que no tienen contraparte en `to`, asi que mapear nbsp→espacio y borrar el resto se hace en una sola llamada.
- **⚠️⚠️ Una regla nueva en una tabla que alimenta un webhook publico crea un veneno permanente si el webhook no pregunta antes.** Un lead de Meta cuyo telefono sea un usuario de WhatsApp golpearia el trigger, el webhook devolveria error, **Meta reintentaria para siempre** y el lead nunca entraria. El webhook llama `telefono_utilizable` **antes** de insertar y desvia el valor a `usuario_whatsapp`. Regla general: **antes de poner una regla en una tabla, buscar quien escribe en ella sin poder manejar un rechazo.**
- **⚠️ Un insert cuyo error se descarta se vuelve peligroso el dia que la tabla empieza a rechazar.** La puerta de negocio nuevo (`negocio-v2-actions.ts`) botaba el error del insert del contacto: con el trigger habria creado negocios **sin contacto y sin avisar**. Se encontro buscando escrituras a `contactos`, no ejecutando.
- **El aviso de pantalla es A PROPOSITO mas estrecho que la regla de la base.** `avisoTelefono` en `phone-input.tsx` solo salta con letras o arroba, para que jamas rechace un numero real a medio escribir; la base es la que decide de verdad. Un test fija esa asimetria. Editar `phone-input.tsx` cubre **6 pantallas** de un solo golpe.
- **⚠️ Cuando se usa un modelo para entender configuracion, el disparo se define por lo que se APRENDE, no por lo que LLEGA.** `entenderFormulario` corre cuando **no hay mapa aprendido para ese `form_id` Y el mapa a mano dejo un hueco real**, no cuando el formulario es nuevo: eso ultimo seria una llamada por cada lead, para siempre, en formularios que legitimamente no preguntan correo. Medido en SOENA: **112 leads en 7 dias contra 5 formularios distintos en 30 dias.**
- **La firma de la funcion es la que protege el dato, no la disciplina de quien la llama.** `entenderFormulario(campos: string[])` recibe **solo los nombres** de los campos, nunca los valores, y no acepta el objeto del formulario: asi la regla de privacidad no se puede romper por descuido. La funcion **nunca lanza**: si el modelo falla o se pasa del timeout (15 s) devuelve null y el webhook sigue con el mapa a mano.
- **⚠️⚠️ Lo que un modelo devuelve y se va a PERSISTIR se valida contra la realidad antes de guardarse.** Un nombre de campo inventado quedaria escrito en `config_extra` del workspace y romperia ese formulario **para siempre, en silencio**. `validarContraElFormulario` compara sin distinguir mayusculas ni espacios pero **devuelve el nombre original** del formulario, descarta todo lo que no este en la lista real, e impide que un campo llene dos papeles.
- **⚠️ Escribir en una columna `jsonb` compartida desde un proceso concurrente se hace con `jsonb_set` en UN solo statement.** Cuando arranca un formulario nuevo los leads llegan en rafaga; un leer-modificar-escribir sobre `config_extra` haria que el ultimo escritor borre a los demas, **en la misma columna donde viven `page_id` y `field_map`**. `guardar_field_map_formulario` lo hace en SQL, verificada contra produccion: dos mapas seguidos, ni `page_id` ni `field_map` ni la config de contacto pisados.
- **⚠️ Gemini 3.x usa `thinking_level`; el `thinkingBudget` numerico de 2.5 se ignora en silencio.** Configuracion usada: `gemini-3.1-flash-lite`, temperatura 0, `responseMimeType: 'application/json'` + `responseSchema` sobre cinco papeles anulables. Verificacion con el modelo vivo: **6 de 6 formularios, cero campos inventados**, ~300 tokens y menos de 2 s cada uno; incluye preguntas en prosa, ingles, aleman y un formulario opaco (`campo_1/2/3`) que devolvio todo null, que es la respuesta correcta.
- **Deuda que dejaron estos dos PRs:** `buscar_contacto_duplicado` **no mira `usuario_whatsapp`**. Un contacto cuyo unico dato sea el usuario de WhatsApp no se puede comparar con nada. Agregarlo como tercera llave toca la firma de la funcion y sus cinco puertas.


**Sesion:** 2026-09-02 (`soena/ve` → producto: **el cash de honorario en tableros va neto de IVA, y una migracion casi se pierde por un numero de version repetido**). PRs **[#502](https://github.com/bi-metrik/metrik-one/pull/502)** (`4298f42`) y **[#504](https://github.com/bi-metrik/metrik-one/pull/504)** (`1c9f839`) mergeados. Migracion `20260902220053_tableros_honorario_neto_de_iva.sql` aplicada por MCP y verificada: dos vistas y siete RPC.

- **⚠️⚠️ Dos archivos de migracion con el MISMO numero de version no fallan: `db push` da el numero por aplicado y salta el archivo en silencio.** `20260902000010` estaba en dos archivos de la misma jornada. **Revisar `supabase_migrations.schema_migrations` ANTES de aplicar**, no despues. Y `apply_migration` del MCP registra la fila con **su propio timestamp**, no con el del nombre del archivo: hay que mirar el ledger despues y renombrar el archivo a la version que quedo, o el repo miente sobre si mismo. Hoy conviven en el repo otras dos versiones duplicadas (`20260902000004`, `20260902000006`) y cinco migraciones aplicadas por MCP que nunca quedaron como archivo (`20260826141500`, `20260826162802`, `20260826211947`, `20260831171338`, `20260901192404`). ✅ Ya lo rechaza el check `Ninguna migracion repite version` (`scripts/check-migracion-version-duplicada.mjs`), pero **solo sobre lo que el PR agrega o renombra**: las 32 colisiones de `main` quedan **congeladas, no corregidas** (la de `20260630000001` dejo `security_advisors_ola1` sin aplicar desde junio).
- **⚠️ Una migracion aplicada a mano se verifica con `md5(prosrc)`, no con un ensayo previo.** Postgres guarda `prosrc` **verbatim**, tal cual va entre los delimitadores `$function$`. Se saca el md5 de cada cuerpo del archivo en local y se compara contra `md5(prosrc)` en produccion. Mide **lo que quedo**, no lo que se envio, que es la pregunta real cuando hay 49.914 bytes retipeados dentro de una llamada MCP. 7 de 7 identicas. `create or replace function` **no** resetea la ACL, asi que los `revoke` del archivo siguen siendo necesarios pero los grants previos sobreviven.
  - ⚠️ **Un md5 que NO coincide no prueba que la logica difiera.** Verificado el 2026-09-09 sobre `registrar_email_alterno` (#565): produccion `9233eb4a`, local `6afab148`. Volcados los dos cuerpos, la diferencia son **solo comentarios y lineas en blanco**, la logica es identica token por token: lo que se aplico salio de un texto con los comentarios despojados. El md5 sirve como **prueba de igualdad**, no como prueba de desigualdad. Si no coincide, hay que volcar y comparar, no concluir.
- **⚠️ `caso_completo` y cualquier comparacion recaudado-contra-vendido tienen DOS lados, y bajar uno solo rompe el indicador.** Medido antes de aplicar: con los dos en base quedan **285 completos de 306**, identico a antes; **bajando solo el numerador habrian quedado 0.**
- **Bajar a base se hace agregando columnas, no reemplazandolas.** `v_cobro_valor` conserva todas sus columnas brutas y gana `a_tramo1_base` / `a_tramo2_base`, porque `v_cartera_negocio`, `v_mc_linea_mes`, `v_pyl_mes`, el gate `saldo_cero` y la conciliacion contra ePayco (`src/lib/upme/imputacion-pago.ts`) leen las brutas y ahi la cifra correcta es la que entra a la cuenta. `v_venta_mes_comercial` si baja en sitio, porque se verifico contra `pg_proc` que sus unicos consumidores son las RPC de tablero. **`a_tarifa` no tiene ni debe tener base: la tarifa UPME no causa IVA, se consigna integra.**
- **⚠️ `get_comercial_perfil_soena` no leia `v_cobro_valor`:** sumaba `cobros.monto` a secas y separaba honorario de tarifa por `tipo_cobro`. El cobro ENTERO se contaba como honorario del comercial, y `tarifa_recaudada` mostraba **cero para todos, siempre**, porque nadie registra cobros con `tipo_cobro = 'pasante'` (la tarifa viaja dentro del cobro y la separan los techos). Contradecia a `get_comercial_resumen_soena`, que ya imputaba bien la misma cifra para la misma persona. **Cuando dos RPC sirven el mismo dato, la que no pasa por la vista de imputacion es la sospechosa.**
- **Metodo, cuando la torre no tiene `SUPABASE_ACCESS_TOKEN` ni `psql`:** la service role key de `.env.local` sirve para PostgREST pero **no para DDL**; la unica via es el MCP. Montar un pasamanos `SECURITY DEFINER` que ejecute texto arbitrario con esa key **no es camino**, y el clasificador lo bloquea con razon.

**Sesion:** 2026-09-02 (`soena/ve` → producto: **una lectura por lote que se cortaba en 1.000 filas sin decirlo, y una marca que se pisaba a si misma**). PRs **[#491](https://github.com/bi-metrik/metrik-one/pull/491)** (`1fe8fd4`) y **[#500](https://github.com/bi-metrik/metrik-one/pull/500)** (`0b4ff57`) mergeados y desplegados. Migracion `20260902000010_corregir_marcas_siigo_cliente_pisadas.sql` aplicada, respaldo en `public.backup_marcas_siigo_cliente_20260902`.

- **⚠️⚠️ Una consulta PostgREST sin paginar se corta en 1.000 filas y NO devuelve error.** `getColaFacturacion` pedia los bloques de todos los candidatos en una sola consulta; hoy necesitaba **1.115 filas**. Las 115 sobrantes desaparecian y la cola concluia que esos negocios **no tenian RUT ni servicio contratado**, pintandolos como incompletos con los datos intactos en la base. **48 negocios con precio aprobado ($28.981.462) invisibles para facturacion**, y en el otro sentido **V0089 y V0428 ya facturados sin que la cola los viera asi — V0428 aparecia listo para emitir**. El riesgo de doble facturacion no era teorico. Cerrado con `traerTodo` en `src/lib/supabase/paginar.ts`, que **o devuelve todo o lanza**: una pagina que cae justo en el limite pide la siguiente, el tamano de pagina se acota al tope real del servidor (pedir 5.000 y recibir 1.000 se leeria como "se acabo") y el `error` se propaga nombrando la consulta. Las ocho lecturas por lote de la cola pasan por ahi con `.order('id')`: **sin orden estable, la pagina 2 no continua donde termino la 1.**
- **⚠️ Cuales filas se pierden en un truncamiento NO es estable: depende del plan.** Dos corridas del mismo arnes separadas por minutos dieron 24 y 23 casos afectados, y la emulacion en SQL cortaba filas distintas de las que cortaba el servidor. **El sintoma no sirve para acotar alcance** — lo que se mide es el conteo de filas de la consulta contra el techo. Las pruebas cubren tres ordenes distintos de siembra por eso.
- **⚠️⚠️ Un `{...entidad.metadata, campo: x}` construido sobre una lectura vieja pierde toda escritura intermedia.** `emitirFacturaNegocio` leia el negocio al principio, llamaba a `asegurarClienteSiigo` (que corrige y **reescribe** `metadata.siigo_cliente` cuando la marca no coincide con el RUT) y al final guardaba la marca de la factura sobre la copia vieja, devolviendo el tercero a su valor equivocado. **12 negocios ya facturados quedaron con la cedula del tercero corta en un digito.** El recibo de caja tenia el defecto identico. Cerrado con `guardarMarcaEnMetadata` en `src/lib/negocios/marca-metadata.ts`, que **relee la metadata justo antes del update**; aplicado en `siigo/facturas.ts`, `siigo/recibos.ts` y `marcar()` de `siigo/clientes.ts`.
- **⚠️ Una pantalla que calcula el dato bueno y pinta el crudo se contradice sola.** La cola armaba el borrador del cliente con la regla correcta (contacto gana, RUT de respaldo) y despues construia la fila con `contacto.email` a secas: **69 de 114 casos mostraban el correo en blanco teniendolo todos en el RUT**, y a la vez la pantalla **no** listaba "email" entre los faltantes. La fila sale ahora del mismo borrador que usa la emision. **No se hizo backfill de `contactos.email` desde el RUT:** la precedencia contacto sobre RUT es deliberada y copiarla la borraria.
- **Metodo:** las pruebas nuevas usan un doble de Supabase que **recorta en 1.000 filas igual que el servidor** y un doble de `asegurarClienteSiigo` que **escribe de verdad** la correccion intermedia; sin eso ninguna de las dos fallas se reproduce. Verificacion contra produccion de solo lectura, con `siigoRequest` sustituido en el arnes de la cola y **cero POST a Siigo** en toda la sesion.

**Sesion:** 2026-09-02 (`metrik-one` → producto: **el trigger de asignacion automatica escribe el `rol` y deja de tapar huecos**). PRs **[#495](https://github.com/bi-metrik/metrik-one/pull/495)** y **[#496](https://github.com/bi-metrik/metrik-one/pull/496)** mergeados. Migracion `20260902000008_trigger_asignacion_escribe_rol.sql` aplicada y registrada en el ledger. Sin cambio de datos: solo `create or replace` de la funcion.

- **⚠️⚠️ Una fila de `negocio_responsables` con `rol` NULL es invisible para el motor de avisos.** `destinatarios_negocio` busca la fila cuyo `nr.rol` corresponde al stage y, si no la encuentra, escala al supervisor del area: el responsable real nunca se entera de su propio caso. Se arreglo por el lado de la aplicacion en agosto (`rolDesdeAreas` / `asignarResponsable`), pero **el camino SQL quedo abierto y lo volvia a sembrar solo**. Medido antes de tocar nada: de 884 filas en toda la base, **158 con `rol` NULL y las 158 con `assigned_by` NULL**. Ninguna era humana. `assigned_by IS NULL` es la firma de "lo escribio el trigger" y es la forma barata de separar defecto de decision.
- **`asignar_responsable_area_entrante()` sabia para que area estaba asignando (`v_area_nueva`) y no lo dejaba escrito.** Ahora el insert escribe el rol del area entrante. **`financiera` sigue entrando con `rol` NULL a proposito: solo existen DOS puestos** (decision de Mauricio del 2026-08-10, documentada en `responsable-rol.ts`), y ese es el limite del modelo, no un defecto.
- **⚠️ En un trigger AFTER UPDATE, una violacion de indice unico tumba la operacion que lo disparo.** Escribir el rol sin comprobar el puesto habria hecho que un negocio **no pudiera cambiar de etapa** por un detalle de asignacion. El `not exists` del puesto libre va **dentro de la sentencia** (`insert ... select ... case when exists (...)`), no en un `if` previo: preferible una fila con rol null que un caso atascado. Regla general: **un trigger de conveniencia nunca puede hacer fallar la transaccion que lo invoco.**
- **La cascada dejo de asignar supervisores, admins y el owner** (decision de Mauricio, 2026-09-02: "que no inserte nada, que avise el cron"). Los pasos 1 (`workspace_default_responsables`) y 2 (operador unico del area) se quedan. **Un responsable puesto por defecto tapa el hueco sin resolverlo**: se veia atendido y no lo estaba. Ahora el negocio entra sin responsable y lo levanta `detectar_responsable_faltante_area()`, que ya existia, es idempotente y tiene la misma cascada de destinatarios.
- **⚠️ Preguntar por el AREA de la persona no es lo mismo que preguntar por el PUESTO que ocupa, y el guard usaba lo primero.** `join staff_areas ... where sa.area in (v_area_nueva,'direccion')` deja que una fila sin rol —que no recibe ningun aviso— alcance para que el trigger se salte el negocio. Pasa a `nr.rol = v_area_nueva`, que es el criterio del motor de avisos; `financiera` conserva el criterio por area porque no tiene puesto que consultar.
- **⚠️⚠️ Medir contra el ARCHIVO de una migracion no es medir contra lo que corre.** El brief de este frente pedia el mismo cambio en `detectar_responsable_faltante_area` con una cifra de "15 negocios empezarian a alertar", sacada de leer `20260520000013`. Esa funcion **ya habia sido reemplazada** el 2026-09-01 por `20260901000011`, que saco la regla a `debe_alertar_responsable_faltante()` y decide por `nr.rol` en los workspaces con `config_extra.notificaciones.routing_por_responsable`. Al medir contra la funcion viva la diferencia real era **2 negocios de un workspace sin opt-in**, no 15. **Antes de proponer un cambio a una funcion SQL, leer `pg_get_functiondef` de la base, no el archivo del repo**: en un repo con varias sesiones en paralelo el archivo mas nuevo no siempre es el ultimo que se aplico.
- **QA sin base local ni branch de pago: todo en UNA transaccion que termina en `rollback`.** El MCP de Supabase respeta el control de transaccion, asi que se puede crear un workspace de prueba entero (con `auth.users`, `profiles`, `staff`, negocios), reemplazar la funcion, correr las transiciones, afirmar y revertir. **12 casos en verde, corridos dos veces** (con la funcion inyectada y despues contra la desplegada). Guion en `proyectos/soena/ve/qa/2026-09-02_qa-trigger-asignacion-rol.sql`. Cuidado con una cosa: se cambia `stage_actual` a mano y **nunca `etapa_actual_id`**, porque ese otro trigger llama una edge function por `pg_net` y **eso no se revierte**.


**Sesion:** 2026-09-02 (`soena/ve` → producto: **el aviso al cliente deja de depender de que el negocio cambie de etapa y puede colgar de un DOCUMENTO**). PR **[#490](https://github.com/bi-metrik/metrik-one/pull/490)** (`2d67fb7`) mergeado con los 5 checks verdes; migracion `20260902000006_aviso_al_cliente_por_documento.sql` aplicada y registrada; edge function `notificar-etapa` desplegada y verificada contra el codigo servido.

- **⚠️ No todo evento que le importa al cliente es una entrada a una etapa.** `trg_avisar_entrada_etapa` cuelga del UPDATE de `negocios.etapa_actual_id`, asi que solo sabe avisar cuando el negocio se mueve. La factura no se mueve con el negocio: medido en SOENA, la etapa "Facturacion" ha visto **10 negocios en toda la vida de la linea** (2 con factura), mientras que de los **185 que si tienen factura la subieron parados en 10 etapas distintas**, y **142 de ellos nunca volvieron a cambiar de etapa despues**. Colgar ese aviso de una etapa lo dejaba disparando antes de que el documento exista (omitido con `sin_link`, que no se ve en ninguna pantalla) o no disparando nunca.
- **`trg_avisar_documento_cargado` sobre `negocio_bloques`** dispara cuando el `drive_url` de un bloque que declara `avisar_al_cliente` **pasa de vacio a lleno**, y `notificar-etapa` acepta `bloque_config_id`: cuando viene, resuelve el copy del **bloque** en vez de la etapa. Todo lo demas queda igual (los tres copys, los corchetes opcionales, `{link}`, la copia al comercial, WhatsApp, la traza).
- **⚠️ La columna va ANTES que la function.** `registrarAviso` no propaga su error a proposito, asi que desplegar la edge function nueva contra una base sin `avisos_cliente.bloque_config_id` habria dejado **todos** los avisos por etapa saliendo bien y **sin traza**, un fallo que no se ve desde ninguna pantalla. Orden obligatorio: migracion, function, y de ultimas la config del workspace que arma el trigger.
- **Cuatro guardas independientes contra un envio masivo a clientes reales, que no se deshace:** es `AFTER UPDATE` y no `INSERT` (los **162 de 185** casos con factura entraron por INSERT de un cargue, ninguno habria disparado); exige **`auth.uid()`**, o sea una persona con sesion (un `service_role` o un `psql` no despiertan el aviso, y la suplantacion de admin tampoco: una sesion de soporte no puede escribirle al cliente sin que nadie lo sepa); el enlace tiene que **nacer** en el update (cambiar un archivo por otro es una correccion nuestra, no una novedad del cliente); y es **opt-in por bloque**, asi que el trigger nace inerte.
- **⚠️ Un ensayo contra produccion que cierra con `commit;` en vez de `rollback;` escribe en produccion.** Paso: quedo un `drive_url` falso sobre la fila real de `factura_emitida` de un negocio. Se revirtio y se verifico a cero (0 residuo, 0 filas en `avisos_cliente`, 0 en la cola de pg_net). **Ningun cliente fue contactado, y la razon fue la guarda de `auth.uid()`**, no la disciplina del ensayo. `pg_net` encola dentro de la transaccion, asi que el `rollback` tambien cancela el POST: el harness correcto existe, hay que terminarlo bien.
- **PostgreSQL no exige EXECUTE para DISPARAR un trigger, solo para crearlo.** Revocarle EXECUTE a `public`/`anon`/`authenticated` a la funcion del trigger (que el check `Toda tabla nueva declara RLS y su grant` si exige) no la apaga. Verificado con `has_function_privilege` y reensayado despues del revoke.
- **⚠️ La guarda de `auth.uid()` tambien deja fuera lo que emite el propio ONE** (medido el 2026-09-02 al cerrar la sesion, leyendo el codigo). `src/lib/siigo/archivar-documento.ts` archiva **todo** documento emitido con `createServiceClient()` y hace **INSERT** cuando la instancia del bloque no existe. Las dos cosas chocan con el trigger, que exige AFTER **UPDATE** y `auth.uid()`: **dos guardas independientes lo bloquean.** Resultado hoy: el aviso de la factura **solo despierta con facturas subidas a mano**; la que ONE emite contra Siigo se archiva sola y el cliente no se entera, y un recibo de caja archivado por la misma via tampoco avisaria. Es el reverso exacto de la guarda que hace imposible el envio masivo: **la misma condicion que impide que un script le escriba a 185 clientes impide que ONE avise cuando el propio ONE emite.** Sin decidir: aflojar la guarda del trigger reabre el riesgo; llamar `notificar-etapa` explicitamente desde el archivado deja el disparo en el codigo que emite, que es lo que un aviso "por documento emitido" realmente pide.

**Sesion:** 2026-09-02 (`metrik-one` → producto: **una alerta de WhatsApp puede salir por plantilla, y un envio que Meta rechazo deja de gastar cupo**). PR **#474** (`2a97637`) mergeado; `wa-alerts` (v37) y `wa-webhook` (v142) desplegadas y verificadas leyendo el eszip del servidor. Sin migracion. **Inerte hasta que exista el secreto `WA_ALERT_TEMPLATES`.**

- **⚠️⚠️ Un 200 de la Graph API NO es una entrega: el fallo puede llegar despues, por el webhook de acuses.** Las tres alertas W25 del 2026-09-01 devolvieron **200 con wamid** y Meta las fallo un segundo despues con **`131047 Re-engagement message`**. Nadie las recibio. **Consecuencia de diseno, que es lo que importa: ningun control sincronico sobre el RESULTADO de un envio puede funcionar** — en el momento de enviar todo se ve bien, y el desenlace solo existe en `wa_envios` despues. El reporte del hallazgo asumia rechazo sincronico y por eso proponia un arreglo que no era posible; se vio mirando `wa_message_id is not null` en las tres filas.
- **⚠️⚠️ Fuera de la ventana de 24 h Meta SOLO entrega plantillas aprobadas, y un cron a las 8 a.m. casi siempre la encuentra cerrada.** O sea que una alerta proactiva por texto libre **no se atrasa: no llega nunca**, y llevaba asi desde que existe `wa-alerts`. Lo destapo `wa_envios` (#470) — antes el rechazo se botaba. La ventana la abre **quien recibe** al escribirle al bot, no el hecho que dispara el aviso: el aviso de "numero desconocido" del #470 depende de la ventana del ADMIN, no la del desconocido que acaba de escribir.
- **⚠️ Un tope consultado UNA vez antes de un bucle no es un tope.** `runW25` consultaba el cupo de 2/dia una vez por workspace y despues mandaba **hasta tres** mensajes; con varios workspaces del mismo dueno habrian sido 3 por cada uno. Por eso salieron 3 con el tope en 2, y nadie lo habia notado en meses. **No estaba en el reporte del hallazgo**: aparecio al contar los mensajes reales contra el tope declarado, no al leer el codigo.
- **⚠️ Un contador de cupo sobre una bitacora que tiene TODO lo saliente necesita filtrar por origen.** El cupo paso de `wa_message_log` (que no sabe como termino el mensaje) a `wa_envios` **excluyendo `failed`/`rechazado`**, pero `wa_envios` incluye tambien las respuestas del bot: sin `origen='alerta'`, quien chatee dos veces se queda sin alertas. Es exactamente el riesgo que la migracion `20260901000003_wa_envios` dejo advertido para el caso inverso, entrando por la puerta de al lado. **Consecuencia aceptada:** mientras no haya plantillas ningun envio consume cupo, asi que los intentos fallidos diarios por numero suben de 2 a ~6 — ninguno llega, pero deja mas filas.
- **⚠️ Dos variantes de un mismo aviso no pueden compartir plantilla.** W33 dice "lleva N dias sin actualizar" o "nunca registraste saldo" segun el dato; con un solo `{{dias}}` el segundo diria **"tiene 0 dias sin actualizar"**. Se partio en dos intents (`W33` y `W33_sin_saldo`). Y una variable declarada que llega vacia **cae a texto libre** en vez de mandar un aviso con un hueco — pero el **cero es un valor**, no una ausencia (probado: la mutacion que usa `!bruto` tumba esa prueba).
- **La politica de plantillas es DATO (`WA_ALERT_TEMPLATES`), no codigo:** los nombres los aprueba Meta en un tramite que no tiene la cadencia de un deploy. Registro vacio = comportamiento identico al de hoy; un intent sin entrada sigue saliendo como texto. Las cinco plantillas por crear estan en `docs/wa-templates.md` con el nombre exacto de cada variable que el codigo **ya publica**, para que declararlas no toque codigo. **Los 10 templates de abril no servian**: cubren la tabla `notificaciones`, ninguno cubre `wa-alerts`.
- **⚠️ `W29` (resumen semanal) NO cabe en una plantilla** y no se forzo: un parametro de cuerpo de Meta no admite saltos de linea, y ese resumen es un arbol de proyectos. Queda como decision abierta escrita (plantilla-empujon vs. restructurar), no como pendiente sin explicar.
- **⚠️ El check `Tipos de edge functions` mira TODOS los `.ts`, incluidos los `.test.ts`.** Una prueba de vitest dentro de `supabase/functions/` importa `vitest` por npm y sin extension `.ts`: Deno no resuelve ninguna de las dos y el check falla con `TS2307`. Se excluyeron los `*.test.ts` del glob — lo que ese check protege es lo que se **despliega**. Y para reproducirlo en local hace falta **`--node-modules-dir=none`**: sin el flag, Deno ve el `node_modules` de npm y `notificar-etapa` falla por resolucion, que es un falso rojo del propio instrumento.
- **Metodo:** el contador nuevo se midio **contra produccion por el mismo camino que usa el codigo** (PostgREST), con un **control** para no leer un `0` que significara "el filtro no parsea": viejo 3, nuevo 0, control (excluyendo solo `read`) 3. Y las 11 pruebas del modulo puro se vieron **fallar contra 4 mutaciones** antes de confiar en ellas.
**Sesion:** 2026-09-01 (`soena/ve` → producto: **ocho tipos de evento que el CHECK rechazaba en silencio, y cinco vias de asignacion de las que solo una anotaba**). PRs **#475**, **#478**, **#479** y **#480** mergeados y desplegados; migracion `20260901000010_activity_log_tipos_vocabulario.sql` aplicada y verificada contra produccion; dos backfills sobre datos de produccion con autorizacion explicita.

- **⚠️⚠️ `activity_log.tipo` admitia 7 valores y el codigo insertaba 15.** Los 8 sobrantes rebotaban con `23514 violates check constraint` y **ningun insert leia su `error`**, asi que la fila desaparecia sin que nadie se enterara. El sintoma es **identico al del caso sano**: el timeline no muestra el evento, y no hay forma de saber que falta. **Medido en produccion: 311 aprobaciones de propuesta desde el 2026-04-15 y CERO filas `propuesta_aprobada`.** Cerrado con fuente unica (`src/lib/activity/tipos.ts` → `ACTIVITY_LOG_TIPOS`), el helper `registrarActividad` que lee el error y lo reporta con contexto, la migracion que reescribe el CHECK completo, y una prueba de contrato que lee el archivo de migracion y falla si las dos listas se separan — sin esa prueba, "hay una sola fuente" seria una afirmacion, no un hecho. **Los ~60 inserts pasaron por el helper, no solo los 8 rotos:** convertir unicamente los que fallan hoy dejaba 45 tragandose errores de FK y de RLS. Ademas una regla de eslint (`no-restricted-syntax`) prohibe volver a tocar `from('activity_log')` de frente.
- **⚠️ Un guardian que lee prosa no lee codigo.** La prueba de contrato del plazo de conservacion (`src/lib/compliance/retencion.test.ts`, PRs #634 y #641) barre el repo **por contenido** y no por una lista de rutas, porque una lista no puede ver el archivo nuevo. Su regex de prosa exige la palabra «años» detras del numero, asi que un `const RETENCION_ANIOS = 5` le era invisible: se le porto el detector de constantes desde `metrik-valida`, donde ese hueco si mordia porque la constante era la que escribia la fecha de borrado en la base. Dos reglas que quedan para cualquier guarda nueva: **enumerar las formas en que el dato puede aparecer** (prosa, constante, columna, configuracion) y decir cual queda fuera, porque lo que queda fuera **sale verde**; y **validar por mutacion** antes de mergear: cambiar el valor, confirmar que la prueba se pone roja Y que nombra la superficie, revertir. El plazo vive en `src/lib/compliance/retencion.ts` y nadie escribe el numero a mano.
- **⚠️ El mismo insert fallaba por DOS razones y arreglar una sola solo le cambia el codigo de error.** Los tres sitios de platform admin pasaban un `profiles.id` a `activity_log.autor_id`, que es FK a `staff(id)`: violaban el CHECK **y** la FK. Al ampliar el CHECK habrian seguido rechazados, ahora con `23503`. **Al reparar un insert mudo, comprobar todas sus restricciones antes de dar el fix por completo.**
- **⚠️ Contar filas de la tabla fuente NO es contar eventos, y la cifra equivocada se propaga a los comentarios.** El primer conteo dio **754** y se escribio en la migracion, en el helper y hasta en `eslint.config.mjs`. 754 son las FILAS de `negocio_bloques` con `aprobado_at`: la **misma** aprobacion esta copiada en ~2,4 bloques por negocio, porque el caso pasa por varias etapas y cada una tiene su bloque "Propuesta economica". Los eventos distintos, contados por par `(negocio_id, aprobado_at)`, son **311 — exactamente uno por negocio**. Corregido en los #478/#479. **Antes de escribir una cifra en un comentario, contar el par que identifica el EVENTO, no las filas que lo mencionan.**
- **⚠️⚠️ Un `not exists` protege re-corridas, NO duplicados dentro del mismo `INSERT ... SELECT`.** El SQL de backfill propuesto deduplicaba con `not exists (select 1 from activity_log ...)`, que **no ve las filas que el propio statement esta insertando**: habria escrito ~2,4 entradas por evento en la primera corrida y recien la segunda habria quedado limpia. La dedup intra-statement va en el `select` (`distinct on (negocio_id, aprobado_at)`), y el `not exists` queda solo para la idempotencia entre corridas. **Los dos hacen falta y protegen cosas distintas.** El mismo SQL ademas seleccionaba `nb.workspace_id`, columna que **no existe** en `negocio_bloques` (hay que unir contra `negocios`).
- **⚠️ Un helper compartido que escribe la fila pero no la ANOTA deja caminos mudos.** `negocio_responsables` la escriben cinco vias (la pantalla, la auto-asignacion al crear, la conversion de una interaccion y tres scripts de cargue) y **solo la pantalla registraba el evento** — y ni esa llegaba, porque usaba `cambio_sistema`, uno de los ocho tipos rechazados. Medido: **699 asignaciones con rol y cero entradas en el timeline.** El #480 mueve el registro **dentro de `asignarResponsable`**, que es por donde entran las cinco, y el helper **resuelve por su cuenta** lo que el evento necesita (workspace, nombres, autor) en vez de recibirlo por parametro: un parametro opcional se omite, y volveriamos a tener caminos mudos. Es la misma leccion que ya obligo a crear ese modulo una capa mas abajo (el `rol` que solo una via escribia).
- **⚠️ Traducir `profiles.id` → `staff.id` para `autor_id` exige acotar por workspace.** El `UNIQUE (profile_id)` de `staff` es **global**: resolver sin filtrar deja a un platform_admin como autor con su staff de otro tenant. Si no resuelve, el autor va **nulo** — mejor sin autor que con el equivocado. Hallazgo relacionado, abierto: **`negocio_responsables.assigned_by` guarda un `profiles.id` en las 311 filas de operaciones**, asi que todo join contra `staff` devuelve vacio y parece que nadie asigno.
- **⚠️ `apply_migration` del MCP registra la migracion con SU PROPIO timestamp, no con el del archivo.** Quedo como `20260901175020` mientras el archivo del repo es `20260901000010`: el ledger decia que el archivo **no** estaba aplicado, asi que el siguiente `db push` lo habria re-corrido. Es idempotente (`drop constraint if exists` + `add`), pero el ledger mentia. Se alineo borrando la fila ad-hoc e insertando la version del archivo. **Tras aplicar por MCP, comprobar `supabase_migrations.schema_migrations` contra el nombre del archivo.** Precedente del mismo patron ya en la base: `wa_envios` figura dos veces por esto.
- **Metodo:** cada backfill se ensayo **completo** dentro de una transaccion con `rollback` —incluida la ampliacion del CHECK dentro del propio ensayo, para poder insertar los tipos que aun no existian— y se comparo el conteo del ensayo contra el de la aplicacion real. Las cinco pruebas nuevas del #480 se vieron **fallar 4 de 5** con el registro desactivado; la quinta afirma que una fila que NO se pudo escribir tampoco se anuncia, asi que pasa en ambos sentidos por construccion. **La verificacion de punta a punta llego sola:** V0455 se asigno a las 21:20 UTC con el codigo ya desplegado y quedo registrada **sin marca de reconstruccion** — una asignacion real anotada por el codigo vivo, no un ensayo.

---

**Sesion:** 2026-09-01 (`soena/ve` → producto: **la configuracion del bono se vuelve mensual, y el detalle deja de contradecir al resumen**). PRs **#466** y **#469** (`2bd52b2`) mergeados y desplegados; migraciones `20260901000006_config_bono_por_mes.sql` y `20260901000007_detalle_bono_por_indicador.sql` aplicadas y verificadas contra produccion.

- **⚠️⚠️ Un upsert PARCIAL sobre una tabla con columnas `NOT NULL` no conserva los valores vigentes: toma los DEFAULT de la tabla.** `config_bono_operaciones_mes` tiene sus 22 columnas `NOT NULL`, asi que el `COALESCE(m.<col>, d.<col>)` de la RPC **nunca cae al default por columna** — o el mes tiene fila completa, o no la tiene. Guardar solo "suspender envio" en un mes sin fila habria **INSERTADO** con los valores de fabrica en horas, pisos y porcentaje del bono, reescribiendo en silencio toda la politica de ese mes. El fix es resolver la fila efectiva (default del workspace, encima la del mes si existe) y escribir la fila **completa**. **Regla general: antes de escribir un upsert parcial, mirar si las columnas son `NOT NULL`** — el `onConflict` protege el UPDATE pero no el INSERT, y el INSERT es el caso que nadie prueba.
- **⚠️ Un mes SIN fila propia cae al valor por defecto, asi que "el mes manda sobre el default" NO congela nada por si solo.** Mover el default movia los bonos de meses ya liquidados. Por eso guardar cualquier cosa **primero le da fila propia a todos los meses pasados** que aun seguian el default (`congelar_config_bono_meses_pasados`, idempotente). **Verificado en vez de afirmado:** se subio `peso_radicacion` del default de 0,20 a 0,40 dentro de una transaccion; junio y julio **no se movieron** (tienen fila), agosto **si**, y esta bien que lo haga porque en Bogota todavia era 31 de agosto y agosto era el mes en curso. Revertido y confirmado que la config quedo intacta. **Al construir un congelado, la prueba es mover la fuente que deberia dejar de afectar, no leer que la tabla existe.**
- **⚠️ Dos funciones que sirven la misma cifra tienen que resolver su configuracion por el MISMO camino.** `get_operaciones_bono_resumen` empezo a resolver la config del mes y `get_operaciones_bono_detalle` siguio leyendo solo el default. Un mes con horas propias habria mostrado un porcentaje calculado con unas horas y una lista de casos juzgada con otras: **casos marcados a tiempo dentro de un indicador incumplido**. El encabezado de esa funcion **ya declaraba** que existia para evitar exactamente eso (era la leccion de la cifra comercial y su drill), y aun asi se desincronizo al cambiar solo una de las dos. **Al agregar una fuente de configuracion nueva, barrer TODAS las funciones que sirven la misma metrica**, no solo la que se esta tocando.
- **⚠️ Los rangos de un campo que mueve plata se validan en el SERVIDOR, no en el formulario.** Una server action exportada es un endpoint alcanzable aunque la pantalla valide bien (leccion ya escrita aqui para `eliminarPorcionPago`). Un `peso_radicacion` de 5 **no falla en la base** (es `numeric`) y multiplicaria el bono por cinco sin que nada avise. Lista blanca de campos + rango por campo + validacion de los enumerados, antes de tocar la base.
- **Panel lateral de un indicador: se abre SIEMPRE, incluido el suspendido y el que no se pudo medir.** Es donde alguien pregunta por que, y una celda muerta lo deja sin respuesta; el panel explica el estado en vez de no aparecer. Una sola consulta trae los cuatro indicadores, asi que saltar entre ellos dentro del panel no vuelve al servidor. Y **el detalle de un indicador de razon lista el DENOMINADOR, no solo el numerador**: mostrar solo las devoluciones deja el "sobre 43" sin nada que lo respalde, y ese denominador es justo el numero que estuvo mal.
- **⚠️ La trampa de identidad puede convivir en UNA sola funcion SQL.** En `get_operaciones_bono_detalle`: `negocio_bloques.completado_por` → `profiles.id` (radicaciones y envios), `activity_log.autor_id` y `reproceso_eventos.atribuido_a` → `staff.id`. Cruzarlas no da error: da una **lista vacia**, que en un tablero se lee como "esta persona no hizo nada".
- **Un permiso que decide si se DIBUJA un boton puede preguntarse al servidor en vez de viajar como prop.** `TabOperacionesPersonas` se monta desde dos lugares (`/tableros` y `/equipo`) y cada uno tendria que acordarse de pasar el flag: el dia que uno se olvide, el boton desaparece sin que nadie lo note. `puedeConfigurarBono()` usa el **mismo** predicado que aplica la escritura, asi que no puede decir una cosa y la accion otra. Cuesta un viaje al montar; el gate real sigue en el guardado.


**Sesion:** 2026-08-31 (`metrik-one--core` → producto: **el staff que vive en otro workspace ya no choca el unique en cada request**). PR **#453** (`5748ce5`) mergeado y desplegado, los tres checks obligatorios contados y en verde. Sin migracion.

- **⚠️⚠️ Un UNIQUE GLOBAL leido a traves de una RLS por tenant produce un error que NO se puede diagnosticar desde dentro del tenant.** `staff_profile_id_key` es `UNIQUE (profile_id)` **global**, no por workspace; la unica policy de `staff` (`staff_ws`, ALL: `workspace_id = current_user_workspace_id()`) acota toda lectura autenticada al workspace actual. Un profile cuyo staff vive en OTRO workspace era **invisible para la lectura y para la relectura del fallback del PR #180, y chocaba igual en el insert**: `23505 duplicate key value violates unique constraint staff_profile_id_key` **en cada request** de `/negocios` y `/negocios/[id]`, con `staffId = null` de resultado (que degrada permisos, autoria y responsables en silencio). Lo produce el switch de workspace del platform_admin: mueve el `profile`, no el `staff`. **Medido en produccion: 1 solo profile en toda la base en ese estado** (Mauricio, profile en `soena`, staff activo en `metrik`), mas 6 profiles sin staff alguno (a esos el auto-alta si les funciona) y 1 con staff inactivo en su mismo workspace (el caso que ya cubria el #180). **Regla general: cuando un constraint es global y la lectura pasa por RLS de tenant, la comprobacion previa al insert va con el service client** — si no, el codigo pregunta en un ambito mas chico que aquel en el que la base decide, y el sintoma es un choque de unique contra una fila que nadie puede ver.
- **La resolucion pasa al service client, y lo que existe en otro workspace no se presta.** Si la primera lectura falla, se busca por `profile_id` a secas (el unique garantiza a lo sumo una fila): en este workspace se usa, activo o inactivo (criterio del #180, ahora sin chocar); en **otro** workspace **no se inserta ni se devuelve ese id** —seria autoria cross-tenant, porque `staff.id` es FK de `activity_log` y de `negocio_responsables`— y `staffId` queda `null`, que era el resultado efectivo de antes, con un `console.warn` descriptivo **una vez por (profile, workspace) por instancia** en lugar del error repetido. Y **el staff no se mueve de workspace en silencio**: arrastraria responsables, areas e historicos.
- **La carrera insert-vs-select tambien queda cerrada:** el alta pasa de `insert` a `upsert` con `onConflict: 'profile_id', ignoreDuplicates: true` (`ON CONFLICT DO NOTHING`). Dos requests concurrentes del mismo usuario ya no producen 23505; el que pierde la carrera recibe 0 filas y relee con el service client.
- **Deuda de modelo declarada, sin migracion:** los datos **no** muestran un modelo "un staff por workspace" (1 caso, y es el platform_admin), asi que el unique global es coherente con lo que hay y **no se toco**. Si algun dia el platform_admin necesita autoria propia en cada workspace, el camino es `UNIQUE (workspace_id, profile_id)` mas revisar cada consumidor que asume un unico staff por profile — decision de Mauricio, y migracion que toca datos de produccion.
- **⚠️ Consecuencia viva que el fix NO resuelve:** mientras el `profile` de Mauricio siga apuntando a `soena` con su staff en `metrik`, su `staffId` es `null` en toda la aplicacion (sin autoria en `activity_log`, sin poder quedar como responsable de un negocio). El error desaparecio del log; la degradacion sigue. Se corrige con el patron que este archivo ya usa dos veces, `update profiles set workspace_id = home_workspace_id`, que es cambio de datos de produccion y por eso quedo **propuesto y no aplicado**.
- **Metodo:** causa raiz medida contra produccion **antes** de escribir codigo (definicion del constraint con `pg_get_constraintdef`, policies con `pg_policies`, y el conteo de profiles con staff en un workspace distinto al suyo); 6 pruebas con doble consciente **de tabla y de RLS**, y **4 se vieron fallar contra el codigo de `main`** antes de confiar en ellas.

**Sesion:** 2026-08-31 (`soena/ve` → producto: **una pagina que espera sus bloques en fila, filtros que tapan la lista, y metas de todo el año**). PRs **#437** (`1dd7828`), **#441** (`a62e2b9`), **#444** (`520c7f9`) y **#455** (`006a3a0`) mergeados y desplegados. Sin migracion.

> Esta sesion corrio **en paralelo** con la de "rendimiento fase 1" de abajo y midio la misma pantalla
> ANTES de que se arreglaran las 242 politicas RLS y de que la instancia subiera a Micro. Las dos
> partes se complementan: alla estaba la causa de fondo del entorno, aqui la forma en que la pagina
> pedia sus datos. Los numeros de mas abajo son el piso viejo, no el de hoy.

- **⚠️ Una pagina con muchos bloques de datos los espera EN FILA si cada uno trae su propio `await`.** `/tableros` encadenaba siete bloques (las tres genericas, rentabilidad, las once consultas del tablero comercial, Direccion, calidad, proceso por seccional y operaciones) y tardaba **entre 8,4 y 11,4 segundos** de render de servidor, con el resto de la app entre 1,2 y 1,7. Ninguno dependia del resultado de otro: todos dependen solo de `modules` y `role`, resueltos antes del primero. Un solo `Promise.all` con el gate de cada rama intacto los deja en el tiempo del **mas lento** en vez de la suma. **Medido despues: 5,3-8,7 s.** Al revisar una pantalla lenta, mirar primero cuantos `await` consecutivos hay antes del `return`: se lee en un minuto y no necesita ninguna herramienta.
- **⚠️ El tiempo de una consulta AISLADA no dice nada; la diferencia contra su media en produccion ES el diagnostico.** `get_comercial_seccional_mes_soena` cuesta **74 ms** medida sola y en caliente, y **3.200 ms de media en produccion**: cuarenta y tres veces mas. Las **seis RPC mas pesadas corridas en secuencia dentro de una sola conexion suman 547 ms en total**, contra los segundos que tarda cada una cuando caen a la vez. **Corolario incomodo: paralelizar una pagina en una instancia pequeña cambia "sumar esperas" por "todas peleando a la vez"**, y por eso la ganancia fue de un tercio y no de un orden de magnitud. Antes de reescribir una consulta, medirla sola **y** mirar su `mean_exec_time` en `pg_stat_statements`: si no se parecen, el problema no esta en la consulta. (La causa de fondo la encontro la sesion de abajo: las politicas RLS evaluandose por fila.)
- **⚠️ Optimizar una vista canonica sin medir la ganancia primero es una apuesta con el criterio de negocio de prenda.** Se reescribio `v_venta_mes_comercial` (la definicion unica de venta) con los cuatro CTE materializados y se verifico equivalencia total sobre el historico completo: **317 filas, cero diferencias con `EXCEPT` en ambos sentidos**. Pero medida en caliente daba **28 ms la original contra 31 ms la nueva**. **No se aplico y la vista de prueba se borro.** El patron que si sirvio: crear la version nueva como vista `_probe` aparte, comparar con `EXCEPT` en los dos sentidos sobre todo el historico, medir, y recien entonces decidir.
- **Filtros secundarios de una lista: colapsados, pero con chips de lo que esta puesto (`src/components/barra-filtros.tsx`, #437).** En `/negocios` los cuatro desplegables (seccional, origen, servicio, responsable) se apilaban a ancho completo y en celular empujaban el primer caso fuera de la pantalla: pesaban mas que la lista que filtran. Ahora viven detras de un boton **Filtros** con el numero de los activos. **La parte que no se puede quitar son los chips**: un filtro escondido y activo deja una lista corta sin explicacion, y el usuario no tiene como saber por que. Reglas puras y probadas en `src/lib/filtros/campos.ts` — incluida esta: **un valor que ya no esta entre las opciones (enlace viejo, responsable que salio del equipo) sigue contando como activo**, porque sigue recortando la lista y hay que poder quitarlo. El componente es reutilizable; `/directorio/contactos` tiene el mismo patron sin migrar.
- **⚠️ Un editor que recibe el PERIODO de un sitio y las CIFRAS de otro escribe en el periodo equivocado (#444).** El modal de metas tomaba `anio`/`mes` del estado del tablero (que cambia con las flechas de mes) y `metasIniciales` de una prop del servidor cargada **una sola vez para el mes en curso**. Navegar a otro mes y guardar **copiaba las metas del mes inicial encima del que se estaba viendo**: no un error de pintado, escritura de datos falsos. Se arreglo de raiz editando el **año completo**, donde cada fila trae su mes y sus valores de la misma consulta. **Regla general: si un editor toma el identificador del registro de una fuente y su contenido de otra, las dos tienen que refrescarse juntas — o no separarse.** De paso: guardar solo las filas que cambiaron (no las doce) evita pisar lo que otra persona acaba de fijar en un mes que aqui ni se abrio, y el campo vacio se maneja como texto porque "sin meta" no es "meta de cero".
- **⚠️ Una fila de totales tiene que decir de que esta hecha, o dos pantallas discrepan por un nombre (#455).** La matriz del tablero directivo sumaba bien (**388 = 380 abiertos + 8 completados**, verificado contra la base) pero la fila total se llamaba **"Cartera abierta"**, y `/negocios` dice 380 abiertos. Los completados entran a proposito (no tienen etapa viva, asi que la RPC los suma por estado a la fila "Proceso terminado"), pero nadie podia saberlo. Se renombro a **"Total en el proceso"** y la nota dice que son abiertos mas terminados, sin perdidos ni cancelados. **Salio del QA en pantalla, no de los tests**: la suma era correcta en los tres niveles y aun asi la pantalla mentia.

**Sesion:** 2026-08-31 (`soena` → producto: **rendimiento fase 1 en produccion + un tipo re-exportado tumbo el detalle de negocio**). PRs **#443** (RLS InitPlan) y **#452** (hotfix) mergeados y desplegados; migracion `20260831000005` aplicada a mano por Mauricio en SQL Editor.

- **⚠️⚠️ En un archivo `'use server'`, `export type { X }` de un simbolo IMPORTADO revienta produccion y en dev no se ve.** El #440 dejo `export type { TipoReproceso, CausaReproceso }` en `reproceso-actions.ts`. El compilador de server actions del build de produccion convierte **todo** export del archivo en referencia de runtime; los tipos se borran al compilar, asi que el modulo quedo exportando una variable inexistente y **todo** render que lo importaba moria al evaluar el modulo: `ReferenceError: TipoReproceso is not defined` (digest 2537040993), visible en la UI como el error rojo generico de Server Components sobre cualquier bloque del detalle. En dev no se manifiesta, y `tsc`, lint y build en CI pasan: **solo lo delatan los logs de runtime de produccion** (`vercel logs <deploy>`). Regla: en archivos `'use server'`, los tipos **declarados localmente** (`export type Foo = {...}`) si se erasan bien y son patron comun en el repo; lo que no se puede es el **re-export specifier** de tipos importados. Fix en #452: el consumidor importa los tipos directo del modulo que los declara.
- **⚠️ 242 politicas RLS invocaban `current_user_workspace_id()` desnuda → Postgres la evaluaba POR FILA.** 5,6M seq scans sobre `profiles` en 34 dias. El advisor de Supabase NO lo marca (solo detecta `auth.uid()` directo). Todas quedaron envueltas en `(select ...)` = InitPlan, una evaluacion por query (#443 con 19 tablas calientes explicitas + `20260831000005` con las 203 restantes generadas desde `pg_policies`; respaldo en `public.backup_rls_policies_20260831`). Junto con el compute (que estaba en **Nano**, 0,5 GB CPU compartida; subio a Micro): `get_comercial_kpis_mes_soena` de **396 ms → 65 ms**. Diagnostico completo con plan de fases 2-4 en `proyectos/soena/ve/2026-08-31_analisis-rendimiento-plataforma.md`.
...
- **⚠️ `supabase/functions/` no lo verifica ningun check de CI, y ya dejo pasar una regresion completa.** `deno check` sobre `main` fallaba con **2 errores** en este archivo (`negocio.etapa_actual_id` no existe en el tipo `Negocio`), entrados con el PR #377 y detectados recien aqui. El baseline "un solo error en las 9 edge functions" medido el 22-ago **caduco solo en tres dias**: un baseline sin check en CI se re-mide, no se cita de memoria. ✅ **Cerrado el 2026-09-01 (PR #472): ya existe el check obligatorio `Tipos de edge functions`**, que corre `deno check` sobre todos los `.ts` de la carpeta. Caduco la ausencia del check, no el habito de re-medir.
...
- **⚠️ Para 24 horas se pasa `hourCycle: 'h23'`, NUNCA `hour12: false`.** En varias versiones de ICU el segundo selecciona el ciclo **h24** y la medianoche sale como `24:05` en vez de `00:05`. Verificado el 2026-09-08 contra `formatFecha` de `src/lib/dates/bogota.ts` (#583): con `h23` nunca aparece `24:05`. El riesgo no se reproduce en el ICU de la torre, pero el que renderiza es el del navegador del cliente, que no es ese. ⚠️ Y el literal de `es-CO` lleva las particulas del patron CLDR `yMMMd`: sale `08 de sept de 2026, 14:54`, no `08 sept 2026, 14:54`. Los "de" no se quitan con opciones, solo componiendo la cadena a mano.
- **⚠️ `formatBogotaFechaCorta` lo comparten SIETE archivos.** Cambiarlo le mete la hora a todos. Cuando una sola pantalla necesita otro formato, el cambio vive en su helper local llamando directo a `formatFecha`, no en el compartido. Y al contar consumidores, `formatBogotaFechaCorta` y `formatBogotaFechaCortaAno` son **funciones distintas**: un grep por substring las mezcla y el conteo sale mal (paso dos veces, #583 y #589).
- **⚠️ PGRST202 es fallo de la CACHE DE ESQUEMA de PostgREST, no de existencia de la funcion.** Una llamada por SQL directo (o por MCP) **no lo detecta nunca**: la funcion esta en `pg_proc` y responde. Para verificar que una RPC nueva quedo utilizable **hay que llamarla por PostgREST**, que es el camino que usa el codigo. Aplica a toda migracion que cambie la firma de una funcion que consume una edge function.
- **⚠️ Una migracion que cambia la firma de una RPC va ANTES del deploy de la funcion que la llama.** Al reves, la funcion nueva llama con los argumentos nuevos, recibe PGRST202, lanza, y **Meta reintenta para siempre**. El #565 lo dice en el propio archivo de migracion y asi se ejecuto: merge 13:06:11Z, funcion v28 13:16:56Z.
- **⚠️ Un webhook firmado no se prueba con un evento sintetico.** Falsificar el HMAC `x-hub-signature-256` no corresponde, un `leadgen_id` inventado muere en el fetch al Graph API **antes** del codigo que se quiere probar, y el intento escribe filas reales en produccion. Se prueba por (a) la RPC de solo lectura por PostgREST y (b) el trafico real en `meta_leads_eventos` (⚠️ ese es el nombre de la tabla, no `meta_leads_log`).
- **⚠️ `git branch --contains <rama> origin/main` NO VERIFICA NADA.** El segundo argumento de `git branch` es un **patron de nombres de rama**, no la referencia contra la que compara. Como no existe una rama local llamada `origin/main`, la salida es vacia con codigo 0 **para cualquier entrada, incluso una rama inventada**: da verde siempre. Lo correcto es `git merge-base --is-ancestor <rama> origin/main` mirando el codigo de salida, y **validar el instrumento con un control que TENGA que dar 1** (una rama viva sin mergear). Corolario: `git branch -d A B` procesa de a una, borra A y despues falla en B, o sea un borrado parcial disfrazado de comando fallido.
- **⚠️ Antes de commitear, confirmar en que arbol estas: `git rev-parse --show-toplevel`.** Si el path termina en un `.claude/worktrees/agente-<hash>` que no creaste tu, no es tuyo. El 2026-09-08 hubo **tres** sesiones escribiendo en el mismo worktree: una creo su rama ahi, y entre su commit y su push otra hizo checkout de una rama distinta en el mismo arbol, asi que el commit cayo encima de la rama ajena y el push subio la propia apuntando a otra cosa. Regla en `.claude/rules/branch-workflow-one.md`, lineas 52 y 64 a 68.
- **⚠️ Una prueba puede verificar una propiedad que el tipo nunca declaro.** En `caso-listo.test.ts` el fixture pasaba `faltan_recibo` por un spread con cast sobre `CasoFacturable`, que no lo tiene: verde para siempre, midiendo nada. Si un fixture necesita un cast para compilar, la prueba y el tipo ya no hablan del mismo objeto (#568).
- **⚠️⚠️ El motor de rutas solo puede leer bloques que le APLICAN al caso.** Un `condition` en un `bloque_config` no es solo de render: si el motor ignora la condicion, un valor que la pantalla nunca habria pedido decide la ruta, y queda invisible para quien podria corregirlo. El filtro vive en `src/lib/negocios/campos-de-routing.ts` y la condicion la resuelve **siempre** la RPC `condicion_cumplida`, nunca una reimplementacion. Corolario: cualquier codigo que arme un mapa de campos de una etapa para decidir algo pasa por ese helper, no por un `Object.assign` propio.
- **⚠️ Un dato ausente no se escribe como respuesta.** Si un calculo (seccional, ciudad, clasificacion) no resuelve, el campo queda **vacio**. Un vacio cae al default y se ve; una afirmacion inventada decide y no se ve. Costo 4 casos en SOENA, 3 de ellos con la respuesta contraria a la real. Ver `cerebro/errores/motor-lee-un-bloque-que-no-le-aplica.md`.
- **⚠️ El auto-init solo carga los `bloque_configs` de `etapa_actual_id`.** Cualquier limpieza o guard que cuelgue de ahi (como `solo_si`) SOLO alcanza a los negocios parados en esa etapa. Los que llegan de un cargue ya pasados quedan fuera de alcance para siempre: si el defecto puede entrar por una puerta lateral, la correccion va donde se toma la decision, no donde se responde la pregunta.
- **⚠️⚠️ Toda server action que escribe en un bloque lleva guard en el servidor** (`src/lib/permissions/guard-negocio.ts`). Lo que la pantalla oculta o deshabilita no protege nada: una action exportada es un endpoint alcanzable con cualquier id. La lista del 2026-06-04 no era exhaustiva; #676-#683 cerraron los items, el cronograma y la aprobacion.
- **⚠️ Lo que el servidor escribe se decide con la config del bloque leida en el servidor, nunca con parametros del navegador.** Paso con la plantilla de items de `inicializarBloqueItems` y con `require_all_dates` del cronograma, que llegaban del cliente (#677, #678).
- **⚠️ Un tipo de bloque nuevo se clasifica en `src/lib/negocios/cierre-bloque.ts`** (`manual` / `criterio` / `accion_propia`). Sin clasificar cae en `manual` sin avisar: `marcarBloqueCompleto` lo cierra con un clic, sin evaluar su criterio ni exigir su accion propia (#680).
- **Quien puede ser aprobador lo decide `puedeSerAprobador` en `src/lib/negocios/aprobacion-bloque.ts`:** owner/admin del workspace y no desactivado en su `staff`. La usan la lista del selector, la designacion y la decision; no se reescribe en ningun componente (#683, #687).
- **`activity_log.contenido` tiene `CHECK (char_length(contenido) <= 280)`.** Un motivo mas largo tumba el INSERT entero y, en un CTE, se lleva por delante el UPDATE que lo acompaña. Es el CHECK que dejo sin registrar la aprobacion de propuesta de V0431 que hubo que reconstruir a mano.

- **`etapa_historial` es legacy y esta vacia.** El historial de etapas vive en `activity_log` (`tipo='cambio_etapa'`, `campo_modificado` en `etapa` o `etapa_actual_id`) y guarda NOMBRES, no ids: cada renombre de etapa rompe la serie. Alias y propuesta de ids hacia adelante en `proyectos/soena/ve/2026-09-03_brief-max-tablero-tiempos.md`.
- **`bloque_locks` es efimero** (`DELETE` al soltar o vencer, sin historial) y es la unica marca real de inicio de trabajo en un bloque (se reclama al tocar un campo editable, con heartbeat). Guardar su historial es el primer entregable del esquema de tiempos por gates (`proyectos/soena/ve/docs/entrega/2026-09-07_esquema-tiempos-por-gates.html`).
- **`staff.horas_disponibles_mes` tiene 88 h para los operativos de SOENA** cuando la jornada legal es 42 h/semana (~160 h/mes): cualquier ocupacion calculada sobre ese campo sale al doble.

- **⚠️⚠️ El vocabulario de un sistema externo NO se enumera a mano en el codigo: cuando el externo renombra, el codigo no falla, deja de encontrar.** Meta renombro los campos de su formulario a fin de julio y rompio dos capas a la vez sin un solo error, un 500 ni un log. (a) La ficha del contacto tenia una lista escrita a mano de cuatro nombres de campo y quedo **en blanco desde julio** — el `field_data` estaba guardado completo en el 100% de las 803 interacciones, o sea que el defecto era de LECTURA, no de captura. (b) El webhook decidia persona natural con `=== 'natural'` contra un valor de configuracion mientras Meta ya mandaba `persona_natural`, asi que **572 contactos nacieron sin `rol` desde agosto**, y una fila sin rol es invisible para el motor de avisos. La forma robusta: **para mostrar, pintar todo lo que llega y decidir solo que callar**; **para decidir, comparar por subcadena sin tildes y en minusculas**, y **abstenerse explicitamente** cuando el texto menciona las dos opciones o ninguna, en vez de caer a un default silencioso (un default silencioso convierte un desconocimiento en una afirmacion). ⚠️ Cuidado con el fragmento que se traga otra cosa: silenciar por `movil` borraria `automovil`. Del lado que muestra, un falso positivo **borra un dato real**; del lado que busca, solo no encuentra — los dos lados usan la misma tecnica y **no pueden compartir la lista**. Sintoma tipico: un bloque que "se ve vacio para algunos registros" y una columna que "a veces viene NULL" son el mismo fallo visto desde dos capas. Detalle: `cerebro/errores/vocabulario-externo-congelado-en-el-codigo.md`.
- **⚠️⚠️ Mergear a `main` NO despliega las edge functions de Supabase.** `.github/workflows/` solo tiene `migraciones.yml` y `pr.yml`: Vercel cubre la app de Next, pero todo cambio dentro de `supabase/functions/` exige un **despliegue manual explicito** y no esta hecho hasta que se verifica en la funcion viva. El PR #543 (arreglo del webhook `meta-leads-webhook`) quedo **inerte** con los checks verdes, el commit en `main` y el deploy de Vercel en OK; el unico sitio donde consta la verdad es el numero de version de la funcion. Desplegar desde un **extracto limpio de `origin/main`** (`git archive origin/main supabase | tar -x -C <tmp>` y desplegar con `--workdir <tmp>`), nunca desde el arbol de trabajo, que puede estar atrasado y subiria el bug con el mensaje de exito de siempre — de paso no se toca el arbol de nadie. Verificar **por capas**, no por el mensaje del comando: que el numero de version subio, que el fuente desplegado contiene lo nuevo y ya **no** contiene lo viejo, y una peticion en vivo (403 = arranco y rechazo bien; **401 = quedo una puerta de `verify_jwt` que no debe existir y se pierden TODOS los leads**, porque Meta no manda JWT; 500 = ni arranca). **⚠️ Corolario de orden: el saneo de datos va DESPUES del despliegue, no antes.** Se corrigieron 572 contactos y se desplego 50 minutos despues: tres leads entraron en esa ventana y nacieron con el mismo defecto que se acababa de sanear. Detalle: `cerebro/errores/merge-no-despliega-la-edge-function.md`.
- **⚠️⚠️ Toda lectura por lote sin paginar tiene un techo de 1.000 filas y falla en silencio.** No es solo `select().in()` grande: cuenta el numero de FILAS que devuelve la consulta, y un bloque con copias heredadas multiplica sin que se note (13 copias por negocio en SOENA). El sintoma es **datos que se ven vacios estando llenos**, que es peor que un error. Usar `traerTodo` de `src/lib/supabase/paginar.ts` y darle siempre un `.order()` estable.
- **⚠️ Nunca construir un update de `metadata` (o cualquier jsonb acumulador) sobre una lectura hecha antes de llamar a otra funcion que tambien escribe ahi.** Es un read-modify-write y pierde la escritura intermedia sin ruido. Usar `guardarMarcaEnMetadata` de `src/lib/negocios/marca-metadata.ts`, que relee justo antes de escribir.
- **⚠️⚠️ Un bloque puede declarar `es_gate` y no detener nada.** Dos causas independientes, y las dos se ven igual desde la pantalla de configuracion: (a) los tipos de **visualizacion** (`resumen_financiero` y compania) traen `can_be_gate: false`, y (b) un bloque `visible` **sin campos `required` nace ya completo** (`bloque_nace_completo`), asi que el gate esta satisfecho antes de que nadie lo mire. Un gate que informa no es un gate: si tiene que frenar, va sobre un tipo que capture (`datos`, `checklist`) y con campos obligatorios de verdad. Medido el 2026-09-02 disenando la linea de Trappvel: el gate de margen real habria sido decorativo.
- **⚠️ `etapas_negocio` no tiene columna `area`: el area se DERIVA del `stage`.** Asignarle a una etapa un area que no corresponde a su stage no falla al cargar — deja a esa persona sin poder avanzar sus propias etapas, sintoma que se lee como problema de permisos. Si hace falta un dueno distinto del que implica el stage, va por `config_extra`, no inventando la columna.
- **⚠️ `sla_horas: null` NO es neutro.** Hay un piso por stage (3 dias habiles en `venta`, 2 en `ejecucion`), asi que una etapa larga por naturaleza —un viaje en curso, una obra— empieza a avisar inactividad todos los dias sin que nadie haya configurado nada. Dejar el tiempo "para que lo ponga el cliente" es dejarlo en el piso, no vacio. Y en el otro extremo: **las alertas cuentan dias habiles**, asi que una ventana de horas (una prerreserva que vence el mismo dia) no se puede vigilar con este motor.
- **⚠️ `checklist_soporte` no tiene una sola instancia en produccion en los 16 workspaces** (medido 2026-09-02), y su "soporte" es **pegar un link, no subir un archivo**. Antes de montar un gate sobre el —tipicamente el comprobante de un pago— comprobar que el tipo hace lo que su nombre promete.
- **Abrir un workspace nuevo son dos scripts, en este orden** (PR #506 y #508, 2026-09-03): `setup-workspace.ts <slug> --nombre "<Nombre>"` crea la fila de `workspaces` mas la carpeta padre de Drive (`{padre}/{Nombre}/Negocios`, y `drive_folder_id` apunta a `Negocios`), y `setup-workspace-users.ts <slug> --archivo <lista.json>` resuelve `auth.users` → `profiles` → `staff` → `staff_areas`. Los dos son **simulacion por defecto**: sin `--apply` no escriben nada. Despues van `preflight-workspace.ts` y la carga de la linea. `setup-drive-workspace.ts` solo hace falta si el cliente pone su propio Drive; sin `config_extra.drive_*` el workspace usa las credenciales globales de MeTRIK. **Nunca por SQL directo:** un workspace armado a mano queda a medias y produce cuentas sin PDF que el emisor descarta en silencio.

- **⚠️ `profiles.role` NO se escribe directo: lo deriva el trigger `trg_sync_staff_role` desde `staff.rol_plataforma`** (`dueno`→owner, `administrador`→admin, `supervisor`→supervisor, `ejecutor`→operator, `campo`→read_only). Escribir `profiles.role` y despues insertar el `staff` deja a todo el mundo como `operator`, porque el default de `rol_plataforma` es `'ejecutor'` y el trigger pisa lo que se acaba de poner. Se paga sobre todo montando datos de prueba: la cascada de asignacion cuenta operadores unicos del area y con todos en `operator` deja de resolver, que se ve como "el trigger no asigna" cuando el defecto esta en el montaje.
- **⚠️ Con varias sesiones en paralelo, el numero de una migracion se elige mirando el LEDGER, no el directorio.** Lo que otra sesion ya aplico a la base todavia no esta en `main`, asi que `ls supabase/migrations` dice que el numero esta libre y no lo esta. Paso el 2026-09-02: dos sesiones tomaron `20260902000007`, la otra ya estaba aplicada y registrada, y hubo que renumerar la de aca a `000008` en un PR aparte (#496). Comprobar `select version, name from supabase_migrations.schema_migrations order by version desc limit 5` antes de nombrar el archivo.
- **⚠️⚠️ Todo insert a `activity_log` va por `registrarActividad` (`src/lib/activity/registrar-actividad.ts`), y su `tipo` sale de `ACTIVITY_LOG_TIPOS` (`src/lib/activity/tipos.ts`).** Una regla de eslint prohibe `from('activity_log')` de frente. Para agregar un tipo: **primero la constante, luego una migracion que reescriba el CHECK completo, y desplegar la migracion antes o junto al codigo que lo inserta** — al reves, el insert rebota y (sin el helper) desaparece sin rastro. El contrato entre la constante y el SQL lo verifica `tipos.test.ts`, que lee el archivo de migracion. Motivo: durante meses el CHECK admitio 7 valores mientras el codigo insertaba 15, y **311 aprobaciones de propuesta nunca llegaron al timeline**.
- **⚠️⚠️ En un backfill, `not exists` protege re-corridas pero NO duplicados dentro del mismo `INSERT ... SELECT`.** El subselect no ve las filas que el propio statement esta insertando. Si la tabla fuente repite el mismo evento en varias filas —caso tipico: un dato copiado a los bloques de cada etapa— hay que deduplicar **en el select** (`distinct on (<clave del evento>)`) y dejar el `not exists` solo para la idempotencia entre corridas. **Los dos hacen falta y protegen cosas distintas.** Corolario de medicion: **contar filas de la tabla fuente no es contar eventos** — 754 filas de `negocio_bloques` eran 311 aprobaciones reales.
- **⚠️ `apply_migration` del MCP de Supabase registra la migracion con SU PROPIO timestamp, no con el del archivo.** El ledger queda diciendo que el archivo del repo **no** esta aplicado, y el siguiente `db push` lo re-corre. Tras aplicar por MCP, comprobar `supabase_migrations.schema_migrations` contra el nombre del archivo y alinear la fila si difiere.
- **⚠️ `jsonb_set` con `create_if_missing` NO crea el nivel PADRE de la ruta, y no avisa.** Solo crea la ultima llave. Si el padre no existe, devuelve el jsonb **sin tocar** y **sin error**: el update corre, reporta filas afectadas y el campo nuevo no esta. **Paso el 2026-09-01** en el backfill de `aprobado_servicio` (SOENA): la marca de trazabilidad se escribia con `jsonb_set(data, '{_reconstruido,aprobado_servicio}', ..., true)` sobre filas donde `_reconstruido` no existia, y quedaron **724 filas con el valor y 0 con marca**. El dato entro y la trazabilidad se perdio en silencio, que es justo lo que la marca existia para evitar. **Usar `||` (concatenacion) para crear la llave**, preservando lo que el padre ya tuviera: `data || jsonb_build_object('_reconstruido', coalesce(data->'_reconstruido','{}'::jsonb) || jsonb_build_object(...))`. **Y toda migracion que escriba una marca de auditoria lleva una guarda que ABORTA si alguna fila tocada se quedo sin ella**: si no, el fallo es indistinguible del exito. Corolario de la familia de gotchas de verificacion: aqui el update "funciono" y escribio la mitad de lo que decia escribir.
- **⚠️ Cuando DOS puertas escriben el mismo dato, el control tiene que vivir en las dos, y la segunda suele nacer sin el.** El honorario de un negocio se fija por `aprobarVersionPropuesta` (elegir plan sobre una version) y por `corregirAprobacion` (escribir el valor correcto sobre una aprobacion ya hecha). La primera validaba el cap de descuento y exigia rol gerencial sobre el umbral; la segunda solo pedia "un numero mayor que cero", asi que corregir era la puerta trasera al tope. Peor: el comentario de `revertirAprobacionPropuesta` **declaraba por escrito** que el cap no se puede saltar "porque `aprobarVersionPropuesta` lo evalua en CADA aprobacion". El razonamiento era correcto y estaba incompleto, y nadie lo noto porque la puerta nueva se penso como "corregir un dato mal leido", no como "fijar el precio". **En la base las dos escriben lo mismo.** Fix (PR #467): la regla salio a un helper puro (`src/lib/propuesta/gate-descuento.ts`) que aplican ambas. **Regla general: al agregar una via alterna para escribir un dato que ya tenia controles, enumerar los controles de la via original y decidir explicitamente cual NO aplica y por que**; y si el control se puede extraer puro, extraerlo, porque dos copias se desincronizan. **Segunda leccion:** el gate necesitaba `precio_base_con_iva` para derivar el descuento, y **184 de las 302** propuestas aprobadas de SOENA no lo traen (nacieron aprobadas en el cargue historico, sin pasar por `generarVersionPropuesta`). Un control que depende de un campo opcional hay que medirlo contra los datos reales antes de darlo por puesto: se habria quedado mudo en la mayoria.
- **⚠️ Un valor que se puede CORREGIR no se recalcula en pantalla desde su origen inmutable.** El bloque `propuesta_economica` congela `aprobado_honorario` al aprobar y `corregirAprobacion` lo reescribe; el motor de plata (`modelo-dinero`) lee ese campo. Pero el banner y el historial de versiones **recalculaban** el valor desde `versiones[n].valor_final_planX`, la version, que es el registro inmutable de lo que imprimio el PDF. Resultado: la correccion se guardaba bien y la pantalla seguia mostrando el numero viejo, que se lee como "no me deja cambiarlo". Medido en produccion el 2026-09-01: **4 negocios** (V0048, V0259, V0422, V0445) mostrando una cifra distinta de la que se les cobra. **Regla: donde exista un campo congelado y editable, la pantalla lee ESE campo y usa el derivado solo como fallback para registros viejos; y cuando los dos difieren, se dice** ("el PDF v1 dice $637.500"). Esconder la diferencia es lo que hace pasar un dato corregido por un dato que no tomo.
...
| 2026-08-31 | La comprobacion previa a un insert contra un UNIQUE **global** se hace con el service client, no con el cliente autenticado | `staff_profile_id_key` es `UNIQUE (profile_id)` global mientras la policy `staff_ws` acota toda lectura al workspace actual: el select y su fallback preguntan en un ambito mas chico que aquel en el que la base decide, asi que el insert choca contra una fila invisible (`23505` en cada request, `staffId` null). Regla general para cualquier constraint global de este producto. PR #453 |
| 2026-08-31 | Un `staff` que vive en OTRO workspace **no se presta ni se mueve**: `staffId` queda `null` | Devolver ese id meteria autoria cross-tenant (`staff.id` es FK de `activity_log` y `negocio_responsables`); moverlo de workspace arrastraria responsables, areas e historicos. El null es el resultado honesto y era el efectivo de antes, ahora con un aviso una vez por instancia en lugar de un error por request. Reparar el vinculo es cambio de datos de produccion y lo decide Mauricio |
| 2026-08-31 | El unique de `staff` **no** se cambio a `(workspace_id, profile_id)` | Los datos no respaldan un modelo de un staff por workspace: **un solo profile** en toda la base en ese estado, y es el platform_admin. Cambiarlo es migracion sobre datos de produccion mas revisar cada consumidor que asume un unico staff por profile. Queda declarado como deuda de modelo, no aplicado |
