...
- **⚠️ Gotcha de convivencia: dos sesiones NO comparten working tree.** Una `git switch` de otra sesión reescribe tu árbol; pasó tres veces (archivos revertidos, y una rama que arrastró commits ajenos). Una sesión = un working tree (`git worktree` para la segunda), toda rama nace de `origin/main` fresco, y revisar `git status` antes de commitear.
...
- **⚠️ DEUDA CONGELADA — el ranking de calidad se calcula en DOS sitios.** El muro lo hace en linea dentro de `calidad_bloque_periodo`; `get_calidad_equipo` delega en `calidad_ranking_periodo`. Medido el 2026-07-29: salida identica byte a byte en las tres ventanas (dia `e1ecd481`, semana `801a51ad`, mes `c23c687e`), asi que hoy nadie ve una contradiccion. **Antes de tocar CUALQUIER cosa del ranking (muro, pantalla del supervisor, umbrales, orden, columnas), primero se unifica y despues se cambia** — la reparacion es que `calidad_bloque_periodo` delegue en `calidad_ranking_periodo`, que es lo que hacia antes de la v6, con el control medido antes y comparado despues. Condicion de cobro acordada: **antes del siguiente cambio que toque el ranking, o despues de la reunion de Regat, lo que ocurra primero.** No se pago de una porque toca el SQL de la pantalla publica que se proyecta y la ventana para detectar una rotura antes de la reunion era corta.
