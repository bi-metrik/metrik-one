...
**Sesion:** 2026-08-10 (`metrik-one--core` → **lo que nace nuevo deja de nacer concedido a `anon`**). PR **#243**, sin mergear. **Las 3 migraciones de `public` YA ESTAN APLICADAS en produccion** (Mauricio dio el OK); la cuarta, la de `cliente_reposteria`, esta escrita y **sin aplicar**.
...
- **⚠️ `cliente_reposteria` sigue abierto: la migracion existe pero NO se pudo aplicar.** Hallazgo lateral del barrido: ese schema arrastra el mismo default permisivo y **ninguna de sus 5 tablas tiene RLS** (en `public` no quedaba ni una asi). Solo `productos` tiene datos, 14 filas. Mauricio dio el OK para cerrarlo y `20260810120300_cerrar_cliente_reposteria.sql` esta escrita y verificada, pero el clasificador de permisos de la sesion bloqueo su aplicacion (tambien bloqueo el `enable row level security` por separado). **Estado real hoy:** perdio TRUNCATE/REFERENCES/TRIGGER por la primera migracion de la tanda, pero conserva SELECT e INSERT para `anon` y sigue sin RLS. Queda aplicar esa migracion a mano.
