...
- **⚠️ Con una captura o confirmación desactualizada la cotización NO sale de borrador** (Mauricio, 2026-09-22): lo niega el SERVIDOR (`captura-desactualizada-datos.ts`) en los dos «Enviar», en «Aprobar» desde borrador y en `updateCotizacion({ estado })`, antes del piso de margen. El botón del editor se apaga con el MISMO texto (`motivoParaNoEnviar`). **No frena:** el PDF (sigue con el aviso), aprobar una ya `enviada`, ni lo que no tiene `tarifa_pax` (ni lee el viaje). Si no puede leer líneas o pasajeros, frena.
