...
**Sesion:** 2026-08-10→11 (`metrik-one--core` → producto: **el build ya no le pide las fuentes a Google, y el hosting dejo de estar en un plan que prohibe cobrar**). PR **#242** mergeado y desplegado.
...
- **⚠️ Tercera ocurrencia del mismo gotcha de grants, y van tres en dos dias.** Ambas tablas nuevas nacieron con `INSERT/UPDATE/DELETE/TRUNCATE` para `anon` y `authenticated`. RLS tapa el DML pero **TRUNCATE no pasa por RLS**: cualquiera con la anon key del bundle podia vaciar la tabla de reprocesos que penaliza su propio bono. `revoke all` ANTES de otorgar, y comprobar con `information_schema.role_table_grants`, nunca leyendo la migracion.
...
- **⚠️ Un `grant` explicito NO quita lo heredado (#192).** La tabla nueva nacio con DELETE y TRUNCATE para `authenticated` por el default del proyecto, y **TRUNCATE no pasa por RLS**: cualquiera con su token podia vaciar una tabla de auditoria completa. Hay que `revoke all` ANTES de otorgar, y comprobar los privilegios reales con `information_schema.role_table_grants`, no leyendo la migracion.
