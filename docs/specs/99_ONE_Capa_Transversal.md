...
vigente: false
nota_vigencia: "Concepto Mi Negocio vigente como tab (rutas /mi-negocio + /config). Referencias a Pipeline [98B] y Proyectos [98C] como consumidores son legacy — hoy consumen modulo /negocios. Ver metrik-one/CLAUDE.md para rutas y tablas actuales."
---
...
**Principio de diseño:** Las secciones se muestran siempre (no es un wizard bloqueante). El usuario puede completarlas en cualquier orden, pero el storytelling de arriba hacia abajo sugiere el flujo natural. Cada sección es un card expandible/colapsable con estado visual (Si ⬜) y badge derecho con resumen.
...
|--------|-----------|-------|-------|
| Si Completo | Todos los obligatorios llenos | | "Completo" |
...
| Estado | Condición | Ícono | Badge |
|--------|-----------|-------|-------|
| Si Configurado | Al menos logo subido | | "Logo + colores" |
...
|--------|-----------|-------|
| Si Configurado | >=1 servicio activo | "X activos" |
...
| Estado | Condición | Ícono | Badge |
|--------|-----------|-------|-------|
| Si Completo | >=3 gastos fijos | | "$X/mes" (suma total) |
...
| Estado | Condición | Ícono | Badge |
|--------|-----------|-------|-------|
| Si Configurado | Cuenta + saldo registrado | | "1 cuenta" |
...
|-----------|---------------|
| 1 | Se muestra solo ficha "YO". Al completar salario + horas, sección = Si |
...
| Estado | Condición | Ícono | Badge |
|--------|-----------|-------|-------|
| Si Completo | X de X personas registradas con salario + horas | | "X personas" |
...
| Estado | Condición | Ícono | Badge |
|--------|-----------|-------|-------|
| Si Configurado | >=1 mes configurado | | "12 meses configurados" o "$15M/mes" |
...
|---------|------|--------|-------|
| Mi perfil fiscal | 3 | Si (1.0) | 3.0 |
| Mi marca | 1 | ⬜ (0.0) | 0.0 |
| Mis servicios | 2 | Si (1.0) | 2.0 |
| Mis gastos fijos | 3 | (0.5) | 1.5 |
| Mi cuenta bancaria | 2 | Si (1.0) | 2.0 |
| Mi equipo | 2 | (0.5) | 1.0 |
| Mis metas | 3 | Si (1.0) | 3.0 |
| **Total** | **16** | | **12.5** |
...
- La barra permanece en 100% con ícono Si
...
│ Tu logo (opcional — lo puedes agregar después) │
│ │
│ ┌──────────────────────────────────────────┐ │
│ │ [Empezar ->] │ │
│ └──────────────────────────────────────────┘ │
...
|---|----------|-----------|---------------|
| D119 | Renombrar tab | De "Configuración" a "Mi Negocio". Ícono: . Subtítulo: "Los cimientos de tu negocio..." | Consenso (Sofía + Santiago + Mateo + Hana) -> Mauricio Si |
| D120 | Barra de progreso global | Fija en header, pesos ponderados, celebración al 100% | Sofía + Hana -> Mauricio Si |
| D121 | Sección Mi marca | Logo (PNG/SVG, 2MB) + color primario + secundario. Preview cotización en vivo | Mateo + Santiago -> Mauricio Si |
| D122 | Fusión Mi Equipo + Personal | Una sola sección. Input "¿Cuántas personas?". Tipos acceso: App/WhatsApp/Ambos | Sofía + Hana -> Mauricio Si |
| D123 | Orden storytelling | 4 capítulos: Identidad -> Operación -> Equipo -> Metas. 7 secciones en orden narrativo | Hana -> Mauricio Si |
| D124 | Onboarding checklist previo | Pantalla bienvenida con lista de lo que necesita tener a la mano. Solo primera vez | Sofía -> Mauricio Si |
| D125 | Eliminar Mi tarifa | Costo hora = salario ÷ horas. Se calcula y muestra inline en Mi equipo con texto educativo | Carmen + equipo completo -> Mauricio Si |
| D126 | 7 secciones finales | Perfil fiscal, Marca, Servicios, Gastos fijos, Cuenta bancaria, Equipo, Metas | Consenso -> Mauricio Si |
| D127 | Contenido detallado + nombres con posesivo | Campos, consumidores, estados, pesos por sección. Nombres: "Mi perfil fiscal", "Mis gastos fijos", etc. | Max + Kaori -> Mauricio Si |
| D128 | Navegación global 5 tabs | Orden: Números -> Pipeline -> Proyectos -> Directorio -> Mi Negocio. Lógica: frecuencia + importancia. Mobile: bottom bar. Desktop: sidebar 3 zonas. Tablet: sidebar colapsada. Primera visita -> Mi Negocio, después -> Números | Consenso (Hana + Sofía + Santiago + Mateo) -> Mauricio Si |
