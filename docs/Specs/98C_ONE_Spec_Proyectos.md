...
version: 2.2
updated: 2026-02-22
depends_on: [21], [99], [98D], [98E]
depended_by: [98A]
decisiones: D68-D97, D101-D102, D105
sesion: Gastos Empresariales — FAB + Sugerencia Gasto Fijo
revisado_por: Max (Tech Lead), Kaori (Documentación), Hana (QA), Vera (COO)
---

# Spec Técnica: Módulo Proyectos + Adiciones Pipeline v2.2
...
Todo lo marcado con 🆕 es nuevo en esa versión.
...
Todo lo marcado con 🆕v2.2 es nuevo en esta versión. Max: busca `🆕v2.2` para implementar solo los cambios incrementales.

| Sección | Cambio |
|---------|--------|
| §2.2 🆕v2.2 | ALTER TABLE gastos: CHECK constraint actualizado con valor `'empresa'` |
| §3.6 🆕v2.2 | `gastos_fijos_config` agrega columna `sugerencia_rechazada BOOLEAN` (D105) |
| §6.4 🆕v2.2 | Sección ampliada con sugerencia automática de gasto fijo recurrente (D105) |
| §7.2 🆕v2.2 | FAB: label "🏢 Gasto de mi empresa" (D101), filtro categorías 6-9 para path empresa (D102), helper text + badge deducibilidad |
| §9 🆕v2.2 | Decisiones D101, D102, D105 añadidas |
| §10 🆕v2.2 | Paso 0 (DB cambios), Paso 5 actualizado, Paso 8 nuevo (cron D105) |
| §11 🆕v2.2 | Verificaciones de integridad y post-ejecución para gastos empresa |
...
|------|----------------------|--------|-------------|
| **Cliente** | "¿Estoy ganando o perdiendo en este trabajo?" | Pipeline → Cotización → Hard gate → Proyecto (automático) | ✅ Facturas + cobros + cartera |
| **Interno** | "¿Cuánto me está costando esta inversión?" | Creación manual con fricción (solo desde tab Internos) | ❌ Bloqueado a nivel de backend |
...
**🆕v2.2 Actualizar CHECK constraint de gastos.tipo (D101-D102):**

```sql
-- Agregar valor 'empresa' al constraint para diferenciar gasto empresa intencional de gasto sin clasificar
...
CREATE TABLE gastos_fijos_config (
  ...
  -- 🆕v2.2 Anti-spam sugerencia automática de convertir a fijo (D105)
  sugerencia_rechazada BOOLEAN DEFAULT false,
...
|-----------|-----------|--------|
| en_ejecucion → pausado | ✅ | No se permiten nuevos registros de horas/gastos/facturas |
| en_ejecucion → cerrado | ✅ | Ejecuta §5.5. Inmutable después. Solo cobros permitidos post-cierre (tipo cliente). |
| pausado → en_ejecucion | ✅ | Se reactivan registros |
| pausado → cerrado | ✅ | Ejecuta §5.5 |
...
### 🆕 §6.4 Protección contra abuso del flujo interno (D94) + 🆕v2.2 Sugerencia gasto fijo (D105)

#### Protección contra abuso del flujo interno (D94)
...
#### 🆕v2.2 Sugerencia automática de convertir a gasto fijo (D105)

**Trigger:** Cron mensual o post-registro de gasto empresa (`proyecto_id IS NULL`).

**Lógica de detección:**

```sql
-- Buscar gastos empresa que se repiten 2 meses consecutivos
SELECT 
  concepto,
  categoria_id,
  COUNT(*) as ocurrencias,
  AVG(monto) as monto_promedio,
  STDDEV(monto) / NULLIF(AVG(monto), 0) as coeficiente_variacion
FROM gastos
WHERE tenant_id = :tenant_id
  AND proyecto_id IS NULL
  AND fecha >= date_trunc('month', CURRENT_DATE) - INTERVAL '1 month'
  AND fecha < date_trunc('month', CURRENT_DATE) + INTERVAL '1 month'
GROUP BY concepto, categoria_id
HAVING COUNT(*) >= 2
  AND (STDDEV(monto) / NULLIF(AVG(monto), 0)) <= 0.20  -- ±20% variación
  AND NOT EXISTS (
    -- Solo sugerir si NO está ya configurado como gasto fijo
    SELECT 1 FROM gastos_fijos_config gfc
    WHERE gfc.tenant_id = :tenant_id
      AND similarity(gfc.nombre, gastos.concepto) > 0.6
      AND gfc.sugerencia_rechazada = false
  );
```
...
│ 💡 Tip                                          │
│                                                 │
│ Detecté que pagas *Arriendo* cada mes           │
│ (~$2.000.000). ¿Lo agrego a tus gastos          │
│ fijos para que quede automático?                │
│                                                 │
│  [Sí, agregar a fijos]  [No, gracias]           │
└─────────────────────────────────────────────────┘
```

**Si usuario es Pro+ — también vía WhatsApp (bot proactivo):**

```
📅 Noté que pagas *Arriendo* cada mes (~$2.000.000).
¿Lo configuro como gasto fijo para que quede automático?

1️⃣ Sí, agrégalo
2️⃣ No, gracias
```

**Si confirma → crear registro en `gastos_fijos_config`:**

```sql
INSERT INTO gastos_fijos_config (tenant_id, nombre, categoria, monto_referencia, activo)
VALUES (:tenant_id, :concepto, :categoria, :monto_promedio, true);
```

**Regla de spam:** Mostrar sugerencia máximo 1 vez por concepto. Si usuario dice "No" → `UPDATE gastos_fijos_config SET sugerencia_rechazada = true WHERE concepto = :concepto`. No volver a sugerir ese concepto.
...
### §7.2 Registro de gasto desde FAB (cualquier pantalla) — 🆕v2.2
...
|-------|------|-------------|----------------|
| Monto | Currency input | Sí | — |
| Categoría | Select | Sí | **Condicional por path (D102)** — ver tabla abajo |
| Proyecto | Select | **No** | 🆕v2.2 Tres opciones: lista de proyectos activos (cliente + interno con badge gris) + separador + **"🏢 Gasto de mi empresa"** |
...
| Soporte | File upload | No | 🆕v2.2 Badge "💡 Guárdalo — es deducible" si path = empresa |

**🆕v2.2 Selector de proyecto — Opciones (D101):**

```
┌─ ¿A qué proyecto corresponde? ──────────────────┐
│                                                  │
│  📂 Remodelación Pérez        [cliente]          │
│  📂 Casa García               [cliente]          │
│  📂 Desarrollo ONE            [Interno] ██       │
│  ─────────────────────────────────────────────  │
│  🏢 Gasto de mi empresa                          │
│                                                  │
└──────────────────────────────────────────────────┘
...
**🆕v2.2 Filtro condicional de categorías por path (D102):**

| Path seleccionado | Categorías visibles | Razón |
|-------------------|--------------------|----|
| Proyecto (cliente o interno) | Todas: categorías 1–9 | El usuario elige cuál aplica al trabajo |
| **🏢 Gasto de mi empresa** | Solo categorías **6–9** (arriendo, marketing, capacitacion, otros impuestos/seguros) | Categorías 1–5 son costos directos de trabajo, no de empresa |

**Helper text (solo path empresa):**
```
"Este gasto se registrará como costo operativo de tu negocio"
```

**Lógica condicional completa:**

```
SI "🏢 Gasto de mi empresa" seleccionado:
  → mostrar solo categorías 6-9
  → mostrar helper text operativo
  → mostrar badge deducibilidad en campo soporte
  → gastos.proyecto_id = NULL
  → gastos.tipo = 'empresa'

SI proyecto seleccionado (cliente o interno):
  → mostrar categorías 1-9
  → campo Rubro aparece
  → gastos.proyecto_id = proyecto.id
  → gastos.tipo = 'directo'

SI sin selección (usuario no toca el campo):
  → gastos.proyecto_id = NULL
  → gastos.tipo = 'operativo'  ← comportamiento anterior preservado
```

**Al guardar (path empresa):**
1. Ejecutar `match_gasto_fijo` (§5.3) con categoría seleccionada
2. Si match → mostrar modal de reconciliación (igual que operativo)
3. Si no match → crear gasto con `proyecto_id = NULL`, `tipo = 'empresa'`
4. Post-registro: evaluar trigger D105 (sugerencia gasto fijo — §6.4)
...
│                                              │
│ Por rubro:                                   │
│ Horas diseño:    $3M / $2.4M (-20%) ✅       │
│ Materiales:      $1.5M / $1.8M (+20%) ⚠️     │
│ Transporte:      $500K / $200K (-60%) ✅      │
└──────────────────────────────────────────────┘
```
...
| 🆕v2.2 FAB "🏢 Gasto de mi empresa" (D101) | NULL | — | NULL | NULL | Gasto empresa intencional (`tipo = 'empresa'`) | Gastos operativos |
...
| **🆕v2.2 D101** | **Label FAB empresa** | **"Sin proyecto (gasto general)" → "🏢 Gasto de mi empresa". Intencional, no residual.** | **3** |
| **🆕v2.2 D102** | **Filtro categorías por path** | **Path empresa → solo categorías 6-9. Path proyecto → categorías 1-9. Enforced en UI + validación backend.** | **3** |
| **🆕v2.2 D105** | **Sugerencia gasto fijo automática** | **Si mismo concepto empresa aparece 2 meses seguidos (±20% monto) y no está en config → ONE sugiere convertirlo a gasto fijo. Máximo 1 sugerencia por concepto. Rechazable con `sugerencia_rechazada = true`.** | **5** |
...
### Paso 0: 🆕v2.2 Modificaciones DB incremental (v2.2)

```
□ ALTER TABLE gastos — recrear CHECK constraint tipo con valor 'empresa' (§2.2, D101-D102)
□ ALTER TABLE gastos_fijos_config ADD COLUMN sugerencia_rechazada BOOLEAN DEFAULT false (§3.6, D105)
```
...
□ Formulario gasto desde FAB con switch condicional (§7.2) — 🆕v2.2 label empresa, filtro cats 6-9, helper text, badge
...
### Paso 8: 🆕v2.2 Cron sugerencia gasto fijo — Sprint 5

```
□ Implementar cron post-registro: evaluar patrón recurrente empresa (§6.4 D105)
□ Notificación in-app modal sugerencia "¿Agregar a fijos?"
□ Lógica sugerencia_rechazada: no volver a mostrar si usuario dijo no
□ Pro+: Bot proactivo sugerencia vía WhatsApp
□ INSERT en gastos_fijos_config si usuario acepta
```
...
| Verificación | Estado |
|-------------|--------|
| Toda tabla tiene RLS + tenant_id | ✅ Todas |
| Toda tabla tiene índices en campos de query frecuente | ✅ |
| Toda FK respeta orden de creación | ✅ §10 en secuencia |
| Todo campo de UI tiene tipo y obligatoriedad | ✅ §7 completo |
| Toda vista cubre las métricas mostradas en UI | ✅ |
| Toda alerta tiene condición SQL verificable | ✅ §6.2 |
| Toda Edge Function tiene trigger y pseudocódigo completo | ✅ §5 |
| No hay dato que la UI necesite sin query que lo produzca | ✅ |
| Clasificación de gastos cubre todos los escenarios | ✅ §8 |
| Cobros post-cierre documentados como excepción | ✅ §6.3 |
| Cotización flash (sin rubros) tiene flujo alternativo | ✅ D82 en §7.1 y §7.2 |
| La tabla horas está completa con schema | ✅ §3.5 |
| horas_estimadas existe en proyectos | ✅ §3.1 |
| costo_hora se calcula desde personal | ✅ §4.1 lateral join |
| proyecto_notas existe para W11 | ✅ §3.9 |
| Trigger de creación proyecto tiene pseudocódigo | ✅ §5.1 |
| 🆕 Campo `tipo` en tabla proyectos con CHECK constraint | ✅ §3.1 |
| 🆕 `presupuesto_total` nullable para internos | ✅ §3.1 |
| 🆕 Campos ROI opcionales en tabla proyectos | ✅ §3.1 |
| 🆕 Índice por tipo+estado para queries de tabs | ✅ §3.1 |
| 🆕 Vista incluye `p.tipo` para filtrar UI | ✅ §4.1 |
| 🆕 Validación backend bloquea factura/cobro en internos | ✅ §5.6 |
| 🆕 Alertas A03-A06 filtradas solo para cliente | ✅ §6.2 |
| 🆕 Protección anti-abuso documentada completa | ✅ §6.4 |
| 🆕 UI tabs separados con queries diferenciadas | ✅ §7.6 |
| 🆕 Creación interna con fricción (confirmación + shortcut) | ✅ §7.9.1 |
| 🆕 Detalle interno sin secciones facturación/cobros | ✅ §7.9.2 |
| 🆕 Cierre interno con comparativo reducido + ROI | ✅ §7.9.3 |
| 🆕 FAB muestra ambos tipos en selector de proyecto | ✅ §7.2 |
| 🆕 WhatsApp rechaza cobros/facturas contra internos | ✅ §5.6 |
| 🆕 Clasificación gastos internos → línea separada P&L | ✅ §8 |
| 🆕 Verificaciones post-ejecución para internos | ✅ §10 Paso 7 |
| 🆕v2.2 CHECK constraint gastos.tipo incluye valor 'empresa' | ✅ §2.2 |
| 🆕v2.2 gastos_fijos_config tiene columna sugerencia_rechazada | ✅ §3.6 |
| 🆕v2.2 FAB label "🏢 Gasto de mi empresa" (D101) | ✅ §7.2 |
| 🆕v2.2 Path empresa muestra solo categorías 6-9 (D102) | ✅ §7.2 |
| 🆕v2.2 Helper text y badge deducibilidad en path empresa | ✅ §7.2 |
| 🆕v2.2 gastos.tipo = 'empresa' cuando path empresa | ✅ §7.2 |
| 🆕v2.2 Cron D105: detección patrón recurrente empresa | ✅ §6.4 |
| 🆕v2.2 Sugerencia_rechazada previene spam (D105) | ✅ §6.4 |

### Verificaciones post-ejecución v2.2

```
□ FAB → "Gasto" → Verificar que opción empresa está separada de proyectos con separador visual
□ Seleccionar "🏢 Gasto de mi empresa" → verificar que categorías 1-5 desaparecen
□ Seleccionar proyecto → verificar que las 9 categorías están disponibles
□ Registrar gasto empresa → verificar gastos.proyecto_id = NULL y tipo = 'empresa'
□ Registrar gasto empresa → verificar que P&L lo mueve a línea "Gastos operativos"
□ Registrar mismo concepto empresa 2 meses → verificar que sugerencia aparece in-app
□ Rechazar sugerencia → verificar que sugerencia_rechazada = true y no vuelve a aparecer
□ Pro+: Aceptar sugerencia vía WhatsApp → verificar INSERT en gastos_fijos_config
```
