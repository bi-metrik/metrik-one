...
## Limpiar OTROS worktrees desde el propio: sin poder mirarlos por dentro

Medido el 2026-09-07. El guard bloquea `git -C <otra ruta> status` y los `for` compuestos, así
que **no se puede inspeccionar el árbol ajeno antes de borrarlo**. Los dos controles se sustituyen
por otros que sí corren desde el worktree propio:

- **Limpieza** → `git worktree remove <ruta absoluta>` **sin `--force`**. Falla si el árbol está
  sucio, así que el comando ES la comprobación. Si sale sin error, estaba limpio.
- **Nada por perder antes de `git branch -D`** → comparar el `HEAD` que da
  `git worktree list --porcelain` contra `gh pr view <n> --json headRefOid`. Si coinciden, no hubo
  commits después del merge con squash. `git branch -d` siempre dará falso negativo con squash;
  el `-D` se justifica con esa igualdad, no con el estado del PR a secas.

`gh` no es git: `gh pr view` / `gh pr list` no los toca el guard.
