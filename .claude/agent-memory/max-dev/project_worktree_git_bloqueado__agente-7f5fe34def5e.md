...
**El camino limpio, confirmado el 2026-09-07 (PR #560):** `gh pr merge <n> --squash` **sin
`--delete-branch`** sale **mudo y con codigo 0** desde el worktree aislado — no intenta el
checkout local, asi que no choca con el `main` del arbol compartido. Despues, tres comandos
planos: `git push origin --delete <rama>`, `git switch <rama-del-worktree>`, `git branch -D
<rama>`. Aun asi se comprueba `gh pr view <n> --json state,mergedAt,mergeCommit`: **la
ausencia de salida no es prueba de merge**, es la firma de esta ruta.
