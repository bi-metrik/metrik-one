 ...
 *
 * 1. **La confirmación se ata al VALOR, no a la fecha.** Se guarda el NIT confirmado; si
 *    después cambia el NIT que el formulario va a imprimir (porque se corrigió el bloque
 *    `rut`, o porque alguien escribió un override en la casilla), deja de coincidir y el
 *    candado se cierra solo. No hay caducidad que calibrar ni tabla nueva que mantener.
 *
...
/** Lo que queda guardado en `negocio_bloques.data.confirmacion_nit`. */
export interface ConfirmacionNit {
  /** El NIT tal como se confirmó, en dígitos. Es la llave de vigencia. */
  nit: string
  /** `staff.id` de quien tecleó. Puede ser null (script con service client). */
  por: string | null
...
export const MENSAJE_CONFIRMACION_VENCIDA =
  'El NIT cambió después de confirmarlo: vuelve a escribir la casilla 5 del RUT mirando el documento'
...
/**
 * El NIT que hay que confirmar, o null si no aplica.
 *
...
/**
 * ¿La confirmación guardada sigue sirviendo para este NIT?
 *
 * Solo el valor decide. Sin fecha de vencimiento: una confirmación de hace tres meses sobre
 * un NIT que no ha cambiado sigue describiendo el mismo acto (alguien leyó ese número en el
 * documento), y una de hace un minuto sobre un NIT que cambió después ya no describe nada.
 */
export function confirmacionVigente(
  confirmacion: ConfirmacionNit | null | undefined,
  nitEsperado: string | null,
): boolean {
  if (!confirmacion || !nitEsperado) return false
  return mismoNit(confirmacion.nit, nitEsperado)
}

/**
 * ¿Falta una confirmación válida para generar? Devuelve el mensaje, o null si puede pasar.
 *
 * Los tres mensajes son distintos a propósito: «nunca se confirmó», «se confirmó y el
 * número cambió» y «el formulario tiene dos NIT» piden tres acciones diferentes, y un solo
 * mensaje genérico obligaría al operador a adivinar cuál de las tres le pasó.
 *
...
export function faltaConfirmacionNit(
  ...
  if (ambiguo) return MENSAJE_NIT_AMBIGUO
  if (nit === null) return null
  if (!confirmacion) return MENSAJE_SIN_CONFIRMAR
  return confirmacionVigente(confirmacion, nit) ? null : MENSAJE_CONFIRMACION_VENCIDA
}

/**
 * Normaliza lo que viene de `data.confirmacion_nit`, que es jsonb libre.
 *
 * Una fila con la forma equivocada (escrita a mano, quedada de una versión anterior) se
 * trata como ausencia de confirmación, no como confirmación válida: el lado seguro de un
 * control es retener.
 */
export function leerConfirmacion(raw: unknown): ConfirmacionNit | null {
  if (!raw || typeof raw !== 'object') return null
  const o = raw as Record<string, unknown>
  const nit = digitosDeNit(typeof o.nit === 'string' ? o.nit : null)
  if (nit === '') return null
  return {
    nit,
