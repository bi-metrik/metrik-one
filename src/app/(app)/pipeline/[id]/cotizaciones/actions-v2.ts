...
export async function getCotizacion(id: string) {
  ...
  const { data } = await supabase
    .from('cotizaciones')
    .select('*, oportunidades(id, codigo, descripcion, empresa_id, empresas(id, nombre, numero_documento, tipo_documento, tipo_persona, regimen_tributario, gran_contribuyente, agente_retenedor, autorretenedor))')
    .eq('id', id)
    .single()
