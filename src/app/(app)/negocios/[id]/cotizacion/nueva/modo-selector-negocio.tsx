...
import { createCotizacionFlashNegocio, createCotizacionDetalladaNegocio } from '../actions'

interface Props {
  negocioId: string
}

export default function ModoSelectorNegocio({ negocioId }: Props) {
  ...
  const handleFlashCreate = () => {
    ...
    startTransition(async () => {
      const res = await createCotizacionFlashNegocio(negocioId, flashDesc, Number(flashValor))
      if (res.success) {
        toast.success('Cotización rápida creada')
        router.push(`/negocios/${negocioId}/cotizacion/${res.id}`)
      } else {
  ...
  const handleDetalladaCreate = () => {
    startTransition(async () => {
      const res = await createCotizacionDetalladaNegocio(negocioId)
      if (res.success) {
