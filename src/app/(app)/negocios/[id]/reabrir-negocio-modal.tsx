  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4">
      <div className="w-full max-w-md rounded-t-2xl bg-white p-5 shadow-xl sm:rounded-2xl">
        <h3 className="text-base font-semibold text-tinta">Reabrir negocio</h3>
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            onClick={onClose}
            disabled={isPending}
            className="flex-1 rounded-md border border-[#E5E7EB] py-2 text-sm font-medium text-tinta hover:bg-papel disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            className="flex-1 rounded-md bg-acento py-2 text-sm font-medium text-white hover:bg-acento-hover disabled:opacity-50"
          >
    ...
    <button
      ...
      className={`flex w-full items-start gap-3 rounded-lg border p-3 text-left transition-colors ${
        checked
          ...
          : 'border-[#E5E7EB] hover:bg-papel'
      }`}
    >
      <span
        className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 ${
          checked ? 'border-acento' : 'border-tinta-suave/40'
        }`}
      >
        {checked && <span className="h-2 w-2 rounded-full bg-acento" />}
      </span>
      <Icon
        className={`mt-0.5 h-4 w-4 shrink-0 ${
          checked ? 'text-acento' : 'text-tinta-suave'
        }`}
      />
      <span className="flex-1">
        <span className="block text-sm font-medium text-tinta">{title}</span>
        <span className="mt-0.5 block text-xs text-tinta-suave">{description}</span>
      </span>
    </button>
  )
}
