...
export default function BloquePagoExterno({
  ...
  const lista = (
    <>
      {pagos.length === 0 && (
        <p className="text-xs text-tinta-suave italic">Sin pagos externos registrados</p>
      )}
      {pagos.map((p, i) => (
        <div
          key={p.cobro_id ?? i}
        ...
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium text-tinta">Ref: {p.referencia}</span>
              <span className="rounded bg-[#EFF6FF] px-1.5 py-0.5 text-[9px] font-medium text-[#3B82F6] border border-[#BFDBFE]">
                externo
              </span>
            </div>
            <p className="text-[10px] text-tinta-suave mt-0.5">{fmtDate(p.fecha)}</p>
      ...
      {pagos.length > 0 && (
        <div className="rounded-lg bg-slate-50 border border-slate-100 p-2.5 flex items-center justify-between">
          <span className="text-[10px] text-tinta-suave font-medium">
    ...
    <div className="space-y-3">
      ...
      {!showForm ? (
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-xs font-medium text-tinta hover:bg-papel disabled:opacity-50 disabled:hover:bg-white transition-colors"
        >
        ...
        <div className="rounded-lg border border-[#E5E7EB] bg-white p-3 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-medium text-tinta-suave">Nuevo pago externo</p>
            <button
              onClick={resetForm}
              disabled={isPending}
              className="text-tinta-suave hover:text-tinta disabled:opacity-50"
              aria-label="Cancelar"
            >
              <X className="h-4 w-4" />
            </button>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <label className="block text-[10px] font-medium text-tinta-suave mb-1">Monto recibido</label>
              <input
                type="text"
                inputMode="numeric"
                value={monto}
                ...
                disabled={isPending}
                className="w-full rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-xs text-tinta focus:outline-none focus:ring-2 focus:ring-acento/15 focus:border-acento disabled:opacity-60"
              />
            </div>
            ...
            <div className="col-span-2">
              <label className="block text-[10px] font-medium text-tinta-suave mb-1">Fecha del pago</label>
              <input
                type="date"
                value={fecha}
            ...
            <div>
              <label className="block text-[10px] font-medium text-tinta-suave mb-1">ReteFuente (opc.)</label>
              <input
                type="text"
                inputMode="numeric"
                value={retefuente}
            ...
            <div>
              <label className="block text-[10px] font-medium text-tinta-suave mb-1">ReteICA (opc.)</label>
              <input
                type="text"
                inputMode="numeric"
                value={reteica}
