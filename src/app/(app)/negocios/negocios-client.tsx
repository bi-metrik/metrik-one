...
const FILTROS: { key: StageFilter; label: string; color: string; bgActive: string }[] = [
...
export default function NegociosClient({ negocios }: { negocios: NegocioResumen[] }) {
      ...
      {current.length === 0 ? (
        <div className="py-16 text-center">
          <p className="text-sm text-muted-foreground">
            {filtro === 'todos' ? 'Sin negocios activos' : `Sin negocios en ${FILTROS.find(f => f.key === filtro)?.label}`}
          </p>
