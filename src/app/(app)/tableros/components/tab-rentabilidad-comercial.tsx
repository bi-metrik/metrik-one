...
// Rampa del acento para rankings (Ren): rank 1 = mas profundo. Se deriva del
// acento para que los ocho puestos salgan distintos; ver rampaAcento.
const RAMP = rampaAcento(8)
...
function KpiCard({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm" style={accent ? { borderTop: `2px solid ${accent}` } : undefined}>
      <p className="text-[11px] font-bold text-tinta-suave uppercase tracking-wide">{label}</p>
      <p className="text-[28px] font-extrabold text-tinta mt-1 leading-none tabular-nums">{value}</p>
    ...
    <div className="space-y-5">
      ...
      <div className="sticky top-14 z-10 -mx-6 px-6 py-3 bg-[#F9FAFB]/95 backdrop-blur border-b border-gray-100 space-y-2">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-500">
          <button onClick={irAnios} className={`hover:text-acento-hover ${filtros.drill === 'anios' ? 'font-semibold text-tinta' : ''}`}>Todos los años</button>
          {filtros.anioScope !== 'todos' && (
            <>
              <ChevronRight className="h-3 w-3 text-gray-300" />
              <button onClick={irMeses} className={`hover:text-acento-hover ${filtros.drill === 'meses' && !filtros.mes ? 'font-semibold text-tinta' : ''}`}>{filtros.anioScope}</button>
            </>
          )}
          {filtros.mes && (
            <>
              <ChevronRight className="h-3 w-3 text-gray-300" />
              <span className="font-semibold text-tinta">{mesLabel(filtros.mes)}</span>
            </>
          )}
        </div>
        ...
        {hayFiltro && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={reset} className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-tinta">
          ...
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={temporal} margin={{ left: 0, right: 8, top: 8 }}>
                ...
                <XAxis dataKey="label" tick={{ fontSize: 10, fill: PALETA.tintaSuave }} tickLine={false} axisLine={false} interval={0} angle={filtros.drill === 'meses' ? -35 : 0} textAnchor={filtros.drill === 'meses' ? 'end' : 'middle'} height={filtros.drill === 'meses' ? 48 : 24} />
                  ...
                  <div key={i} className={`group flex items-center gap-2 ${dim ? 'opacity-40' : ''}`}>
                    <button onClick={() => clickVendedor(v.vendedor)} aria-pressed={sel} className="flex-1 min-w-0 text-left" title={`Filtrar por ${v.vendedor}`}>
                      <div className="flex items-center justify-between text-xs mb-0.5">
                        <span className={`truncate ${sel ? 'font-bold text-tinta' : 'text-gray-600'}`}>{v.nombre}</span>
                    ...
                    <button onClick={() => router.push(`/equipo/vendedor/${slugVendedor(v.vendedor)}`)} title={`Ver perfil de ${v.vendedor}`} className="shrink-0 rounded-md p-1 text-gray-400 hover:text-acento-hover hover:bg-acento-tinte">
...
function Chip({ label, onClear }: { label: string; onClear: () => void }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold" style={{ backgroundColor: 'rgba(16,185,129,0.10)', color: PALETA.acento, border: '1px solid rgba(16,185,129,0.25)' }}>
      {label}
      <button onClick={onClear} className="hover:text-tinta" title="Quitar filtro"><X className="h-3 w-3" /></button>
    </span>
  )
}
