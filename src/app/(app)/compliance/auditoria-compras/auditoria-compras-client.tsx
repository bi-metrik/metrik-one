...
const ESTILO_VEREDICTO: Record<VeredictoCompra, string> = {
  contratada_pese_a_rechazo: 'bg-tinta text-white border-tinta',
  hallazgo_sin_liberacion: 'bg-[#FEF2F2] text-[#B91C1C] border-alerta/30',
  sin_consulta: 'bg-[#FEF2F2] text-[#B91C1C] border-alerta/30',
  consultada_despues: 'bg-[#FFFBEB] text-[#B45309] border-advertencia/30',
  sin_resultado: 'bg-[#FFFBEB] text-[#B45309] border-advertencia/30',
  cubierta: 'bg-[var(--acento-tinte)] text-acento border-acento/30',
  sin_hallazgo: 'bg-papel text-tinta-suave border-[#E5E7EB]',
};
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-tinta">Auditoría de contrataciones</h1>
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-5 space-y-4">
        ...
        >
          <Upload className="h-5 w-5 mx-auto text-tinta-suave" />
          <p className="text-sm text-tinta mt-2">
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-papel text-left text-xs uppercase tracking-wider text-tinta-suave">
            <tr>
            ...
            {informe.filas.map((f) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-tinta">
                  ...
                  {f.compra.referencia && (
                    <div className="text-xs text-tinta-suave">{f.compra.referencia}</div>
                  )}
                </td>
                <td className="px-4 py-2 text-tinta-suave">{f.compra.comprador ?? '—'}</td>
      ...
      {informe.invalidas.length > 0 && (
        <div className="bg-white rounded-lg border border-advertencia/40 p-5 space-y-2">
          ...
          <ul className="text-sm space-y-1">
            {informe.invalidas.map((f) => (
              <li key={f.posicion} className="flex justify-between gap-3 border-t border-[#E5E7EB] pt-1.5">
                <span className="text-tinta">Fila {f.posicion}: {f.eco}</span>
