    ...
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-tinta">Segmentación SARLAFT</h1>
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-5 space-y-4">
        ...
        <select
          ...
          className="w-full max-w-md h-11 px-3 rounded-lg border border-[#E5E7EB] focus:outline-none focus:border-tinta bg-white text-sm"
        >
        ...
        {preset !== 'personalizado' && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-tinta hover:text-acento"
          >
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-5 space-y-4">
        <p className="text-xs uppercase tracking-wider text-tinta-suave font-semibold flex items-center gap-2">
          <AlertTriangle className="h-3.5 w-3.5 text-advertencia" />
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 h-11 px-5 rounded-lg bg-acento text-white font-semibold text-sm hover:bg-acento-hover disabled:bg-[#9CA3AF] disabled:cursor-not-allowed transition-colors"
          >
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 h-11 px-5 rounded-lg border border-[#E5E7EB] text-sm font-semibold text-tinta hover:bg-papel transition-colors"
          >
        ...
        {mensaje && (
          <div
            className={`p-3 rounded-lg text-sm flex items-center gap-2 ${
              mensaje.tipo === 'ok'
                ? 'bg-acento/10 text-[var(--acento)] border border-acento/30'
                : 'bg-alerta/10 text-[#B91C1C] border border-alerta/30'
            }`}
          >
...
function Aviso() {
  return (
    <div className="bg-papel border-l-2 border-tinta-suave rounded p-4">
      <p className="flex items-center gap-2 text-[11px] uppercase tracking-wider font-bold text-tinta-suave mb-1.5">
...
function PanelUniverso({
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <span
            className={`text-xs font-mono font-semibold px-2 py-1 rounded ${
              sumaOk ? 'bg-acento/10 text-acento' : 'bg-alerta/10 text-[#B91C1C]'
            }`}
          >
            Suma: {sumaPct} %
          </span>
        ...
        {Object.entries(variables).map(([k, v]) => (
          <div key={k} className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-tinta">{labels[k] ?? k}</span>
              <span className="text-xs font-mono font-semibold text-tinta-suave">{Math.round(v * 100)} %</span>
            </div>
            <input
              type="range"
              min="0"
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-5 space-y-4">
        <p className="text-sm font-bold text-tinta">Umbrales de clasificación</p>
        <UmbralRow
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs font-mono font-semibold text-tinta-suave">{valor.toFixed(2)}</span>
        </div>
        {!readonly && (
          <input
            type="range"
      ...
      <div className="flex items-center gap-2">
        <label className="text-xs text-tinta-suave whitespace-nowrap">Revisión cada</label>
        <input
          type="number"
          min={1}
          max={36}
          ...
          className="w-16 h-8 px-2 rounded border border-[#E5E7EB] focus:outline-none focus:border-tinta text-sm"
        />
        <span className="text-xs text-tinta-suave">meses</span>
      </div>
    </div>
  );
}
  ...
  if (!distribucion || (distribucion.contrapartes.total === 0 && distribucion.empleados.total === 0)) {
    return (
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-8 text-center text-sm text-tinta-suave">
        <p className="font-semibold text-tinta mb-1">Distribución por nivel de riesgo</p>
        <p>
  ...
  return (
    <div className="bg-white rounded-lg border border-[#E5E7EB] p-5">
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm font-bold text-tinta">{titulo}</p>
        <p className="text-xs text-tinta-suave font-mono">{valores.total} con score</p>
      </div>
      {valores.total === 0 ? (
          ...
          {niveles.map(n => {
            ...
            return (
              <div key={n} className="flex items-center gap-3">
                <span className="text-xs font-semibold text-tinta w-12">{labels[n]}</span>
                <div className="flex-1 h-3 bg-papel rounded-full overflow-hidden">
                  <div
                    className={`h-full ${colores[n]} transition-all`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
                <span className="text-xs font-mono text-tinta-suave w-16 text-right">
                  {v} ({pct}%)
                </span>
              </div>
            );
