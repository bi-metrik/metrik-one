  ...
  if (riesgos.length === 0) {
    return (
      <div className="rounded-lg border border-[#E5E7EB] bg-white p-12 text-center">
        <ShieldAlert className="mx-auto h-10 w-10 text-tinta-suave mb-3" />
        <p className="text-sm font-medium text-tinta">Sin riesgos registrados</p>
        <p className="mt-1 text-xs text-tinta-suave">Agrega el primer riesgo para comenzar la matriz.</p>
      </div>
    )
  }
          ...
          <div key={r.id} className="rounded-lg border border-[#E5E7EB] bg-white overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="font-mono text-xs font-bold text-acento">{r.codigo ?? '—'}</span>
                <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${CATEGORIA_COLORS[r.categoria] ?? ''}`}>
                  {r.categoria}
                </span>
                <span className="text-sm font-medium text-tinta line-clamp-1">{r.descripcion}</span>
              </div>
              <div className="flex items-center gap-2 shrink-0 ml-2">
                <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${NIVEL_COLORS[r.nivel_riesgo] ?? ''}`}>
                  {r.nivel_riesgo}
                </span>
                <span className="text-xs text-tinta-suave">{rCausas.length} causa{rCausas.length !== 1 ? 's' : ''}</span>
              </div>
            </button>
                        ...
                        <tr key={c.id} className="transition-colors hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2.5">
                            <Link href={`/riesgos/causa/${c.id}`} className="hover:underline">
                              <p className="text-tinta line-clamp-2 text-sm">{c.descripcion}</p>
                            </Link>
                          </td>
            ...
            {!isCollapsed && rCausas.length === 0 && (
              <div className="px-4 py-4 text-center">
                <p className="text-xs text-tinta-suave italic">Sin causas identificadas para este evento</p>
              </div>
            )}
          </div>
        )
