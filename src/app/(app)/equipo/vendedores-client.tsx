...
export default function VendedoresClient({ vendedores }: { vendedores: VendedorResumen[] }) {
  ...
  function SortIcon({ col }: { col: SortKey }) {
    ...
    return sortDir === 'desc'
      ? <ArrowDown className="h-3.5 w-3.5 text-acento" />
      : <ArrowUp className="h-3.5 w-3.5 text-acento" />
  }
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                ...
                <th className="py-3 px-4 text-right">
                  <button onClick={() => toggleSort('ventaNeta')} className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-500 uppercase tracking-wide hover:text-tinta">
                    Venta neta <SortIcon col="ventaNeta" />
                  </button>
                </th>
                <th className="py-3 px-4 text-right hidden sm:table-cell">
                  <button onClick={() => toggleSort('utilidad')} className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-500 uppercase tracking-wide hover:text-tinta">
                    Utilidad <SortIcon col="utilidad" />
                  </button>
                </th>
                <th className="py-3 px-4 text-right">
                  <button onClick={() => toggleSort('margenPct')} className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-500 uppercase tracking-wide hover:text-tinta">
                    Margen <SortIcon col="margenPct" />
                  </button>
                </th>
                <th className="py-3 px-4 text-right hidden md:table-cell">
                  <button onClick={() => toggleSort('documentos')} className="inline-flex items-center gap-1 text-[11px] font-bold text-gray-500 uppercase tracking-wide hover:text-tinta" title="Número de facturas y notas de venta del vendedor">
                ...
                <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-tinta text-white text-xs font-bold shrink-0">
                  ...
                  <td className="py-3 px-4 text-right">
                    <Link href={`/equipo/vendedor/${slugVendedor(v.vendedor)}`} className="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-semibold text-acento hover:bg-acento-tinte whitespace-nowrap">
                      Ver más
                    </Link>
                  </td>
                </tr>
